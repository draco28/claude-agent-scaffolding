...
_board_regex_for_key() { printf '^%s ' "$(printf '%s' "$1" | sed 's/\./\\./g')"; }
...
_board_actual_issue() { # $1=project $2=key ; echoes JSON of the matching issue or "null"
  board_cli_issues_list "$1" "$(_board_regex_for_key "$2")" || return 1
...
board_sync() { # $1=ws-or-any-dir [--force] [--bind IDENT]
  ...
  if ! board_cli_project_get "$project"; then
    ...
    if ! board_cli_project_get "$project"; then
      echo "board: '$project' was created as a generic space but is not a Tracker project. Create project '$project' in Tracker with type '$BOARD_SPACE_TYPE' by hand, then rerun /board:sync --bind $project."
      return 6
    fi
  fi
  ...
  while IFS=$'\t' read -r from to; do
    ...
    if ! jq -e --arg t "$tid" '.[] | select((.target // .targetIssue // "") == $t and (.type // .relationType // "") == "is-blocked-by")' "$BOARD_CLI_OUT" >/dev/null; then
