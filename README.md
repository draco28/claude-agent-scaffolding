...
| [`ossify`](./ossify/) | v1.0.0 | Project-level (continuous) | Skeleton-first replacement lifecycle for `scaffold-onboard` + `scaffold-dev` — install one lifecycle or the other, not both. 6 native skills, an implementer subagent, and the standalone utility commands (`/ossify:handoff`, `/ossify:handoff-resume`, `/ossify:work-pr` — Claude Code surface; the OpenCode bundle exposes the skills only, #131). In the Claude and Codex marketplaces as of v1.0.0: the boundary audit (5 dimensions) and the consolidated eval (10 acceptance scenarios) are both closed. The two pilots are operator-owned and post-v1. OpenCode bundle installability begins only after an immutable bundle tag is published. |
