...
only after an immutable bundle tag is published. `code-judo` is not in the bundle
at all — and because an unrecognised name in the plugin allowlist makes the whole
adapter config fail to load rather than skipping that one plugin, do not add it to
the tuple. `.opencode/INSTALL.md` carries the inventory that governs.
