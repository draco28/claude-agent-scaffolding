...
| [`ossify`](./ossify/) | v0.5.0 | Experimental OpenCode opt-in (non-marketplace) | Skeleton-first replacement lifecycle with 6 native skills, an implementer subagent, and the standalone utility commands (`/ossify:handoff`, `/ossify:handoff-resume`, `/ossify:work-pr` — Claude Code surface; the OpenCode bundle exposes the skills only, #131). Experimental installability begins only after an immutable bundle tag is published. It is not in the Claude or Codex marketplace and remains gated from stable v1 by Plan D's consolidated eval and two pilots. |
...
├── ossify/                            # experimental OpenCode opt-in (v0.5.0)
├── docs/
