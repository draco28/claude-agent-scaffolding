...
| [`scaffold-onboard`](./scaffold-onboard/) | v0.3.7 | Project-level (run-once) | Onboarding plugin (skill-first). 8 skills + 4 slash commands. 10-phase guided conversation authors `MASTER-SPEC.md`; derivation produces an 11-file memory-bank, tiered `CLAUDE.md`, managed Codex section in `AGENTS.md`, and governance docs. Publishes a structured `project-roadmap.json` with 3-part `VS-N.M.K` ids and explicit `sprint_id` via workspace-init's routed `well_known_paths.roadmap_state`. Slice demos are agent-judged acceptance checks (#44). |
...
├── scaffold-onboard/                  # scaffold-onboard plugin (v0.3.7)
