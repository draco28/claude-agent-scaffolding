...
|---|---|---|---|
| [`workspace-init`](./workspace-init/) | v0.1.2 | Project-level (run-once) | Bootstrap a dual-repo workspace (AI workspace + canonical) with pairing manifest, routed `well_known_paths`, and AI-trace commit-msg filter. Run-once plugin; first in the scaffolding chain. 2 skills (`initializing-dual-repo-workspace`, `pairing-canonical-repo`), 2 slash commands (`/init-workspace`, `/pair-workspace` via `$ARGUMENTS` bridge). Pairing manifest schema v1.0 at `<ai-workspace>/.workspace/pairing.json`; `well_known_paths.roadmap_state` routes scaffold-onboard's structured roadmap for scaffold-dev field reads. Scenario A migration (`--pair-with`). |
...
├── scaffold-onboard/                  # scaffold-onboard plugin (v0.9.2)
├── scaffold-dev/                      # scaffold-dev plugin (v0.9.0)
