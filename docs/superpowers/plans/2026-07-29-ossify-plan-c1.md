...
**New skills:** `skills/work-item/` (SKILL.md + **6** references — 4 from T7, +2 from T8), `skills/close/` (SKILL.md + **9** references — 3 each from T9, T10 and T11 — the split is planned up front because `close` is strictly bigger than slice close: it carries class-scoping, bone-touch, risk-gate escalation, the patch lane and release close, and `plan-spine`/`start` are already at 499/500 with zero headroom).

> **Counts corrected 2026-08-01, and the failure mode named.** Both were wrong: `work-item` said 4 (T7's own count, never updated when T8 added `round-orchestration.md` + `handoff-contract.md`), and `close` said 8 against 3+3+3=9 in the Files lines of T9/T10/T11. **This line counts only the originating task and silently drops later additions to the same directory** — so re-derive it from the per-task Files lists rather than trusting it, and update it whenever a task adds a reference. `close/SKILL.md` is written by four tasks (T9, T10, T11, T12) and carries a **≤~320-line budget at T9** to leave them headroom.
...
**Design note (D3), state it in the prose so a reviewer does not "fix" it back — and note its SCOPE:** spec §6.1 and the #109 policy say spec-aligned **disposition** recommendations auto-apply, and only load-bearing escalations reach the user. That is a deliberate behavioural change from the source, which surfaces-and-waits at every decision boundary.

> **D3 scope corrected 2026-08-01.** This note previously read "`close` auto-applies", unqualified. **D3 is about disposition triage** (plan line 58), and the work-item close layer this task builds has **no disposition step** — dispositions arrive with the critic findings at *spine* close (T10). Worse, the unqualified claim contradicts this task's own text twice: Step 2 item 4 says "present the recovery menu, and **stop — no auto-select**" and Step 3 says "The recovery menu, **surfaced and never auto-selected**". Both are correct; the blanket "close auto-applies" is not. Keep D3 as a note about the *disposition* rows, and state plainly that **the work-item gate's recovery menu is never auto-selected** — a failed gate is a human decision, not a spec-aligned recommendation.

**Files:** Create `ossify/skills/close/SKILL.md` (**≤~320 lines** — see the budget note below), `references/{routing,work-item-close,impl-check}.md`, `ossify/commands/close.md`, `ossify/tests/test-close.sh`.

> **Line budget added 2026-08-01.** `close/SKILL.md` is written by **four** tasks — T9 (this one), T10 (§5), T11 (§6) and T12 (the harvest pointer) — and no task stated a target, while the global cap is ≤~500. `plan-spine` and `start` both landed at exactly 499 and every later edit to them now needs a paired trim, which is the trap this budget exists to avoid. T9 takes **≤~320**, leaving ~180 for the three later writers. If T9's body runs over, move depth into `references/`, which is what they are for.

**Interfaces:**
- Consumes: `oss id_parse` (T3), `oss manifest_require|doctor|state_restore|repo_root` (T3/T4), `oss verify_acs|verify_step|report_cross_check` (T5), `oss work_item_status` (T1), `oss get` (T3); and Task 8's producer contract — `work_items[].branch` / `.worktree_path` written by `work_item_exec`, plus the `complete`-mode return's `report_path` (`work-item/references/returns.md`).
- Produces: a work item at status `complete` with its `work/<wi-id>-<slug>` branch merged into the spine branch. Consumed by Task 10's binding-order step 1 ("all work items `complete`, else refuse") and step 2 (the spine→canonical merge).

> **Interfaces block added 2026-08-01.** Every other task in this plan carries one (lines 148, 439, 747, 1075, 1359, 1671, 1953, 2157); T9 had none, despite sitting on the seam between T8's producer and T10's consumer — the exact seam whose breakage the Task 8 sweep found.
...
2. **Routing.** The scope is derived **mechanically from the id's shape**, never asked. Call the **dispatcher verb** `oss id_parse <id>` — skill prose cannot reach a bare lib function, because `bin/oss` dispatches only `oss_cmd_*`. It echoes a space-separated line whose **first field is the scope** and whose remaining fields are the numeric components: `work_item 1 2 3` / `spine 1 2` / `release 1`. **Route on the first field only.** An unparseable id exits **rc 1 with empty stdout and empty stderr**, so the one-line error is the skill's to emit — say what was passed and what the three shapes look like. With no argument, refuse and list the open ids rather than guessing (`oss spine_list` / `oss get` with a status filter, as `plan-spine` does) — a close run against the wrong scope is expensive and silent. **Full routing rules — the id grammar, the no-argument refusal, and the anti-patterns — in `references/routing.md`.**

> **`routing.md` was an orphan.** T9's Files list creates it, but no step pointed at it from `SKILL.md`: routing was handled inline in this body item. **Task 13 check 5 (line 2141) fails the suite on any `references/*.md` not pointed at from its own `SKILL.md`**, and a pointer from a sibling reference does not count. Both this file and `impl-check.md` (whose only pointer was from `work-item-close.md`, authored in Step 2) now get a `SKILL.md` pointer. This is the same defect Task 8 shipped with and had corrected — **check every created reference against the body's pointer list before dispatch, every time.**

> **Routing contract corrected 2026-08-01, three ways.** (a) It named the bare lib function `oss_id_parse`; prose must call `oss id_parse`. (b) It said the function "returns `work_item` / `spine` / `release`" — it returns the shape **plus the numeric components** (`bash ossify/bin/oss id_parse r1.s2.w3` → `work_item 1 2 3`), so prose written to the stated contract invites an equality test against the bare word that fails. (c) An unparseable id produces **no message at all** (rc 1, empty stderr — `id.sh:22-24`), so "is a one-line error" describes a behaviour the lib does not provide and the skill must supply.

3. **Pre-flight, common to all three scopes.** `oss manifest_require` (refuse naming `/init-workspace` and `/pair-workspace`); `oss doctor` must be green on schema and replay — a close that mutates a drifted state compounds the drift, and `oss state_restore` is the remedy to name. **Resolve every path to an absolute one up front and never `cd`.** Probe with `oss state_path` and `oss repo_root <key>` and carry the results; the manifest walk starts at `$PWD`, and the 33 no-arg `_oss_resolve_state` call sites in `commands.sh` each re-walk it, so a `cd` mid-ceremony silently re-points the state file rather than failing.

> **Pre-flight discipline corrected 2026-08-01.** "Resolve the state path once, before anything `cd`s" is not enforceable as written — the dispatcher re-resolves on every no-arg call, so there is no single resolution to hold. The enforceable rule is *never `cd`; carry absolute paths*, which is what Task 8's shipped lane already does. Do **not** "fix" this with `export OSS_STATE_FILE` — it changes resolution precedence for every nested call and is not what the other lanes do.

4. **Work-item close** → `references/work-item-close.md`, whose gate is `references/impl-check.md`.
5. **Spine close** → **Task 10's** references.
...
1. **Resolve the three paths, then read `report.md` off disk.** The report *content* is deliberately not in the return payload — but `report_path` **is**, and it is guaranteed absolute and ending in `report.md` (`work-item/references/returns.md` §2). Two routes, and the prose must give both because `/close <id>` is supported standalone:
   - **In the round flow**, take `report_path` off the `complete` return. `spec.md` is its sibling (`work-item/SKILL.md:253` authors the report next to `spec.md` and `handoff.md`), so `spec="$(dirname "$report_path")/spec.md"`.
   - **Standalone**, reconstruct: `ai_root="$(oss repo_root ai_workspace)"`, then the work item's directory is `<ai_root>/docs/specs/<release-id>/<spine-id>-<slug>/work-<wi-id>/`. **`oss spine_dir` returns a RELATIVE path** (`id.sh:33`) and needs the *spine* slug, which is not in state — recover it by globbing `<spine-id>-*`, exactly as `round-orchestration.md` already mandates. Do **not** try to recover it from `work_items[].branch`: that carries the *work-item* slug, which `spine_dir` cannot consume.
   - **If neither route yields an existing `spec.md`, halt naming the id.** `oss_verify_parse_acs` returns rc 2 on a missing spec (`verify.sh:11`), and a silently wrong path fails the gate for the wrong reason.

> **Path resolution added 2026-08-01.** The gate's `oss verify_acs <spec>` and `oss report_cross_check <report> <spec>` were bare placeholders with no instruction for obtaining either path, while `close` is bound to an id only. **Nothing in state holds a spec path, a report path, or a slug** — a live fixture's work item is `{spine,title,target_repo,status,created_at,id,branch,worktree_path,base_sha}`. This is the same class as the `$slug` P0 the Task 8 sweep found, in the same task, one paragraph away from the text that resolves its sibling.

2. **The gate**, per `references/impl-check.md`.
3. **Prove there is something to commit.** The gate ran against the **working tree**; the commit runs against the **index**, and they are not the same thing. The implementer returns `stage_status: "all_staged" | "partial" | "none"` — on `none`, and on any empty `git -C "<worktree-abs>" diff --cached --name-only`, **halt**: a green gate over an empty index means the work is not where the commit will look for it. `partial` is a halt too unless the report explains it. Never `git add` on the implementer's behalf — staging is its job and a silent re-stage hides the discrepancy.
4. On green: `oss work_item_status <id> complete`, then the orchestrator commits **in the worktree** (the implementer stages; the orchestrator owns the commit boundary), **then merges `work/<wi-id>-<slug>` back into the spine branch Task 8 checked out**:
...
5. On any failure: **halt**, surface the source-tagged errors, present the recovery menu, and **stop — no auto-select**. (This is not in tension with D3: D3 governs *disposition* rows at spine close, not a failed gate.)
6. Worktree cleanup does **not** happen here — it happens at spine close, and it is the **last** step there. The reason is the branch, not the report: `oss_worktree_remove` runs `git branch -d` and **refuses an unmerged branch rc 8** (`worktree.sh:112-113`), so cleanup can only succeed after this layer's merge has landed, and running it early converts a recoverable state into a halt.

> **Cleanup rationale corrected 2026-08-01 — the stated cause was false.** This item read "harvest reads `report.md` out of the worktree, so removing it first destroys the harvest source." **`report.md` does not live in the worktree.** `work-item/SKILL.md:253` authors it next to `spec.md` and `handoff.md` in the work item's docs directory under the ai-workspace, which worktree removal never touches. The ordering (cleanup last, after harvest) is still right, and T12's harvest still needs the report to exist — but the reason is branch-merge ordering, and a plan that defends a correct order with a false mechanism invites a reviewer to "fix" the order when they discover the mechanism is wrong. Task 10's own cleanup step states the ordering without a rationale ("Harvest — before cleanup, always"; "Worktree + branch cleanup … Only now"), so it needs no edit — but **T10's Step 1 must not import this reason when it authors `spine-close.md`.**

- [ ] **Step 3: Author `impl-check.md`** — the gate itself:

- **Layer 1, `auto:` ACs, halt-on-first-fail** (the opposite of the implementer's own non-halting verification, and for the opposite reason: the orchestrator gate stops at the first real failure because there is a human to route to). `oss verify_acs "$spec"` then `oss verify_step "$worktree" "<cmd>" "<expectation>"` per row — using the `$spec` and `$worktree` resolved in Step 2 item 1 and read from `work_items[].worktree_path`, never bare placeholders. **`verify_step` already applies the zero-tests guard for you**, automatically and *only* on an `exit 0` expectation (`verify.sh:77-80`): a command naming a recognized test runner whose output shows a zero-tests result fails as vacuous green. Do not re-implement it in the ceremony, and do not expect it on a non-`exit 0` expectation.
- **Layer 2, report cross-check:** `oss report_cross_check "$report" "$spec"`. A missing `auto:` AC halts — an unreported AC is indistinguishable from an unimplemented one. The lib emits `oss: report does not account for: <AC>` at rc 1; the `[report cross-check]` prefix below is the **skill's** surfacing convention layered on top, not a string the lib produces.
...
- [ ] **Step 4: Test + commit.** `tests/test-close.sh` covers the two things a bash test can actually reach, and the task says plainly which is which.

  **(i) The mechanical verbs the router's prose names, through `bin/oss`** — build the fixture the way `test-verify.sh` and `test-worktree.sh` do (a real git repo + a `pairing.json` rooted so the manifest walk finds it from the test's cwd; `run-all.sh` does not `cd`, so tests inherit the repo root): `oss id_parse` returns the right **first field** for each of the three shapes and **rc 1 with empty output** for an unparseable id; `oss report_cross_check` on a report omitting an `auto:` AC returns **rc 1 and names the missing AC** (assert the lib's real message, `oss: report does not account for:` — **not** the `[report cross-check]` prefix, which is the skill's surfacing convention and appears in no lib); `oss verify_step` halts on the first failing row.

  **(ii) Nothing else.** State plainly that **the router itself, the three-layer gate's ordering, the halt semantics and the recovery menu are prose contracts with no executable surface** — Task 13's bash-block harness checks that every `oss` verb they name resolves, and beyond that they have no automated coverage in C1. A bash test asserting "each id shape routes to its own scope" would be testing `oss id_parse`, which `test-id.sh` already covers, not the router. Do not write an assertion whose subject is a fixture you authored to satisfy it.

  Run `bash ossify/tests/run-all.sh`, then commit `ossify/skills/close ossify/commands/close.md ossify/tests/test-close.sh`.

> **Step 4 corrected 2026-08-01.** Two of its four assertions were unachievable as written and a third was a duplicate. `[report cross-check]` is emitted by **no lib** (`oss_verify_report_cross_check` emits `oss: report does not account for: <AC>`), so a test asserting that literal fails; "each id shape routes to its own scope" cannot reach the router because the router is prose; "a green work item reaches `complete`" re-tests `oss work_item_status`, already covered in `test-entities.sh`. The step also named **no fixture** and stated **no prose-only carve-out** — the convention the corrected Task 8 Step 4 now sets.

  The `commands/close.md` wrapper follows the shipped shape of the four in `ossify/commands/`: the `ARGS_FROM_CLAUDE="$ARGUMENTS" bash -c '…'` env-var bridge (**never** positional `$1`/`$2` — Claude Code substitutes those at template-render time and silently corrupts them), `allowed-tools` pinned, then a bare `Skill(ossify:close)`. **Single-quote the frontmatter `description`** — it will contain `": "` in its trigger-phrase list, and an unquoted YAML scalar containing `": "` is exactly what left two shipped `agents/*.md` files in this repo unparseable.

**Named risks:** (1) **who calls this?** — does anything route *into* `close`, and does `plan-spine`'s prose already promise a close contract this task must honour? Grep it. And the reverse: name the line where each of `oss id_parse`, `verify_acs`, `verify_step`, `report_cross_check`, `manifest_require` and `state_restore` gains its first non-test caller. (2) Is routing genuinely mechanical from the id, using the **dispatcher verb** and routing on the **first field only**? (3) Does the pre-flight carry absolute paths and never `cd`, and does the gate use resolved `$spec`/`$worktree`/`$report` rather than placeholders? (4) Is the halt genuinely terminal — no subsequent step runs? (5) Does the prose scope D3 to *disposition* rows, and is the recovery menu explicitly never auto-selected? (6) Is worktree cleanup correctly *absent* from this layer, with the **branch-merge** reason rather than the false harvest-source one? (7) Do **all three** references get a pointer from `SKILL.md` itself — `routing.md`, `work-item-close.md`, `impl-check.md`? A pointer from a sibling reference does not satisfy Task 13 check 5. (8) Is `close/SKILL.md` ≤~320 lines, leaving headroom for T10/T11/T12? (9) Does the commit step verify the index is non-empty before committing?
...
2. **Switch canonical back to its base branch, then** merge the spine branch to it, **halting on conflict** (spec §6, "merge halt-on-conflict"):
   ...
   # DERIVE the spine branch; never read it off HEAD. Slug recovered from the
   # spine directory name (round-orchestration.md:88-93); base_branch from the
   # plan doc's `## 2. Spine context` (handoff-contract.md:58).
...
> ### The `rev-parse HEAD` correction — the P0 this sweep found (2026-08-01)
>
> This block previously read `spine_branch="$(git -C "$canonical" rev-parse --abbrev-ref HEAD)"   # T8 parked us here`, taking the spine branch off ambient HEAD and certifying the assumption in a comment. **T8 does park canonical there, but that is durable git state a session boundary, a hotfix, a user checkout or a T9 halt can move.** When it moves, `spine_branch` reads as the *current* branch, the switch-back is a no-op, and the merge is "Already up to date" at **rc 0**.
>
> Reproduced in a sandbox with canonical left on `master`: `spine_branch` read as `master`, merge rc 0, the spine commit never landed, and the spine's file was absent from the tree. **Nothing halts** — steps 3-11 then run apply-pending, the cumulative demo, the critic, the retro and `oss spine_status <spine> closed`, all green, against a tree the spine never reached.
>
> **This inverts a guard Task 9 already shipped.** `work-item-close.md:170-173` derives the branch and asserts HEAD matches it, and `:192-197` states the reason verbatim: *a merge onto the wrong branch succeeds silently at rc 0.* T8's own consequence table (`round-orchestration.md:74-76`) predicts the same outcome. T10 dropped both that assertion and T9's post-merge reachability check.
>
> **The branch assertion is the load-bearing leg — reachability alone cannot replace it.** On a self-merge `$pre` is trivially its own ancestor, so `--is-ancestor` returns 0. Keep both: the assertion catches the wrong-branch case, the reachability check catches a merge that reports success without landing the tip.
...
5. **Bone-touch check (mandatory on flesh, core on bone).** **Compute the changed-path list first** — it is the merge's own diff, `git -C "$canonical" diff --name-only "$base_branch".."$spine_branch"` (the merge has landed by now, so `HEAD` works too). Then `oss touch_check $paths` — **unquoted, word-split into one argument per path**. **rc 0 = HIT, rc 1 = clean, rc 2 = could-not-check.** Reading this backwards is the single most consequential mistake available here. **An empty path list returns rc 2, not rc 1** (`registries.sh:51`) — a spine that somehow changed nothing is inconclusive, never clean. rc 2 is **not** clean — halt and say why. A hit reclassifies the spine mid-flight: `oss class_set "$spine_id" bone "<reason naming the bone's ADR ref that touch_check printed>"` (3 args — the reason is required and is the audit trail for a mid-ceremony class change), then **re-enter the checklist from step 5** on the bone path so the bone-only rows above this point are not skipped.

> **Step 5 corrected 2026-08-01.** `<changed paths>` was a placeholder no step computed, so the row could not be invoked; `oss class_set` was the only verb in the checklist given with no argument shape and no reason, though `commands.sh:28` requires three; and the rc-2-on-empty-input behaviour was unstated, which turns "the spine changed nothing" into a silent halt with no explanation. The re-entry question ("switches to the bone close path" — from where?) was also open, and matters: a late reclassification that continues forward silently skips the bone-only rows it just became subject to.

6. **Risk-gate escalation:** a touch of a recorded risk gate's surface escalates to the bone path **plus that gate's controls**, regardless of class. Harm is orthogonal to reversibility. `touch_check` prints `risk_gate <name>` for a gate hit (distinct from `bone <adr>`), and the gate's controls are the `controls` array recorded by `oss risk_gate_add` (`commands.sh:34`, `$3=controls-csv`) — read them with `oss get '.risk_gates[] | select(.name=="<name>") | .controls'` and walk each as a checklist row. **Distinguish the two hit kinds by the printed prefix**, not by assuming a hit is a bone.

7. **architect-critic — the flag differs by class, and one string cannot serve both.** `oss critic_detect`; **absent → exactly ONE warning, then proceed** (a silent skip and a blocking error are both wrong; note the verb prints `absent` and **returns rc 1**, so guard the call rather than letting it abort the ceremony). Then, per class:
   - **bone** → `export ARCHITECT_CRITIC_ARGS="--spec \"<abs path>\" --close"` — `--close` is precisely what recruits the external Codex adversary at close depth.
   - **flesh** → the same export **without `--close`**, which is the light host-only pass.

   Then a bare `Skill(architect-critic:critiquing-spec)` — no `target=`/`depth=` parameters, both failure modes silent. **`--spec` takes one quoted absolute path to a real file**, not a "bundle": point it at the spine's plan document under `oss spine_dir` (the same artifact `plan-spine` hands the critic). A path that does not resolve triggers the glob fallback and audits the wrong artifact, silently.

> **Step 7 corrected 2026-08-01, two defects.** It named a bone/flesh split and then gave **one** invocation string carrying `--close` for both — but `--close` *is* the depth switch (`critiquing-spec/SKILL.md:3,103`), so the flesh path as written recruited the external adversary, which is the opposite of "one light host-only pass". And `--spec "<abs bundle path>"` named no artifact that exists; "bundle" appears nowhere in the critic's interface.

8. **Retrospective** — authored against a section contract (D6), full for bone / lean for flesh. **Step 3 must define both section sets**; neither the spec nor any shipped file provides them, so "the bone and flesh section contracts" is an instruction to invent, not to port. State the sections explicitly in `retrospective.md` the way T7 pinned the ten report sections.
...
**Files:** Modify `skills/close/SKILL.md` (§5, **plus three new `references/` pointers** — see Step 5); create `references/{spine-close,cumulative-demo,retrospective}.md`; extend `tests/test-close.sh`.

> **Pointer instruction added 2026-08-01 — this is the THIRD consecutive task with this defect.** T10 creates three references and the only SKILL.md change it scoped was "(§5)". **Task 13 check 5 fails the suite on any `references/*.md` not pointed at from its own `SKILL.md`**, and a pointer from a sibling reference does not count. T8 shipped this defect, T9 shipped it again with two orphans, and T10 had it a third time. **Standing rule for every remaining task: diff the created-references list against the SKILL.md body's pointer list before dispatch.** `close/SKILL.md` is 251 lines at HEAD against a ~320 target set at T9 and a ~500 cap; T10 should land **≤~400**, leaving T11 and T12 their share.

- [ ] **Step 1** Author `spine-close.md` (the ordered checklist above, with the bone/flesh column from spec §6.1 rendered as a table and the "core rows are never skippable in either class" guarantee stated). **Do not import the false cleanup rationale**: cleanup is last because `oss_worktree_remove` runs `git branch -d` and refuses an unmerged branch rc 8 — **not** because harvest reads `report.md` out of the worktree. It does not; `report.md` lives beside `spec.md` in the docs dir (`work-item/SKILL.md:253`). **Step 2** Author `cumulative-demo.md` — the auto/user split with the §6.1-vs-§6.2 scoping made explicit, the halt discipline, quarantine handling (skipped by the runner, still owed by the next release close), and the ledger wall-clock budget read from `releases[].ledger_budget`. **Step 3** Author `retrospective.md` — **define** the bone and flesh section contracts (they exist nowhere yet; see step 8 above).

- [ ] **Step 4: Test the mechanical seams this task owns — and say which contracts have none.**

  **(i) What a bash test can reach.** Extend the fixture `tests/test-close.sh` already builds: **a spine close invoked with canonical parked on a branch other than the spine branch halts before the merge** (assert the halt, and assert the spine commit is **NOT** reachable from `base_branch` afterwards — the rc-0 self-merge is exactly what stays green otherwise); `oss touch_check` with **zero paths** returns **rc 2, not rc 1**; a bones-unreadable state returns rc 2; a clean control returns rc 1. Assert **concrete observable values**, not just rc codes.

  **(ii) What it cannot.** State plainly that **"apply-pending runs before the demo" and "a failing demo halts before critic/harvest/cleanup" are orderings of prose steps with no executable surface** — Task 13's harness checks that every `oss` verb they name resolves, and beyond that they have no automated coverage in C1. Writing a bash script that calls apply-pending then demo_run asserts nothing about the ceremony; it tests a fixture authored to pass. Do not write it.

  Note `t_capture` is `T_OUT="$("$@" 2>&1)"` — a command substitution that forks a subshell and contains every `cd`, so any assertion about ambient state written through it cannot fail.

- [ ] **Step 5: Wire the pointers, run the suite, commit.** Add **three** pointers to `close/SKILL.md` — one per new reference, matching the shipped one-pointer-per-reference convention T9 established (`routing.md` §2, `work-item-close.md` §4, `impl-check.md` §4). `spine-close.md` belongs in §5, which currently stands as a stub. Confirm `close/SKILL.md` is **≤~400**. Then `bash ossify/tests/run-all.sh`, the root-parity suite and the eval gate, and commit `ossify/skills/close ossify/tests/test-close.sh`.

**Named risks:** (1) **who calls this?** — is `apply_pending` actually invoked, and *before* the demo? Trace it; T2 built it with no caller. And the reverse: name the line where `oss_reg_touch_check`, `oss_entity_set_spine_class`, `oss_cmd_critic_detect`, `oss_demo_user_lines`, `oss_demo_record_close`, `oss_worktree_remove` and `oss_entity_set_spine_status` each gain a first non-test caller; if any gains none, record it rather than inventing one. (2) Is `touch_check`'s rc read correctly in all **four** cases — hit, clean, could-not-check, **and empty input (rc 2)** — and is the changed-path list actually computed? (3) Does the demo scope match spec:271 (this spine's own `user:` lines) rather than all of them? (4) Is the halt genuinely terminal at every layer? (5) Is cleanup strictly after harvest, and is the stated **reason** the branch-merge one rather than the false harvest-source one? (6) Is the architect-critic bridge **class-dependent** — `--close` on bone, absent on flesh — and does `--spec` name a path that resolves? (7) Does any prose reuse eval-fixture wording? (8) Do **all three** references get a pointer from `close/SKILL.md` itself? (9) Is the spine branch **derived and asserted**, never read off HEAD?
...
1. **All spines closed** — refusal gate, naming any that are not. Enumerate with `oss get '[.spines[] | select(.release == "<rel>")]'` and check each `status` against `closed` (the spine enum is `planned|active|closed|abandoned`; decide and state whether `abandoned` satisfies the gate — it is not `closed`, and a silent pass here would close a release over abandoned work).
2. **Full cumulative walkthrough** — the human drives **every** accumulated `user:` line (`oss demo_user_lines` with **no argument**, which returns the whole active set across spines), against the **amended** set. The spec groups the walk **by feature** (spec:296) — but **demo-ledger lines carry no feature field** (`ledger.sh:37` writes `{type,text,outcome,source_spine,status,status_reason,status_by,at}`), so the grouping is the agent's, derived from `source_spine` and the feature map (`oss feature_list`), not a field it can sort on. Say so rather than implying a lookup that does not exist.
3. **Blocking finding — fake expiry:** every **outstanding** `fakes[]` entry — `status == "active"` **or** `status == "renewed"` — whose `expiry_release` is **at or before** this release is a **blocking close finding**. Resolve by replacing it (`oss fake_status <b> replaced "<reason>"`) or **explicitly renewing** it with a new expiry and a stated reason (`oss fake_status <b> renewed "<reason>" <new-expiry>`). It cannot be ignored — deferred truth never becomes permanent silently.

   **Separately and non-mechanically:** a fake whose `replacement_trigger` has fired is *also* a blocking finding, but that arm is **agent judgment, not a selector** — see the split below.

> **The comparison is AT-OR-BEFORE, and it must be NUMERIC — corrected 2026-08-02.** This row said "whose `expiry_release` **is** this release", i.e. identity. **Identity lets every fake that outlived its own deadline escape forever.** Proven on a live fixture holding three outstanding fakes with expiries `r1`, `r2`, `r2`, evaluated at `r3`'s close: the equality selector returns `[]` — all three escape and the close is silently green — while at-or-before returns all three. That is the same silent-escape shape as the `renewed` defect boxed above, on the other axis: one leaks by status, this one leaks by release.
>
> **And a string comparison is wrong at `r10`.** jq evaluates `"r2" <= "r10"` as **false** (lexicographic), so the helper must strip the `r` and compare numerically: `(.expiry_release | ltrimstr("r") | tonumber) <= ($rel | ltrimstr("r") | tonumber)`. The ID grammar makes `r<N>` unbounded, so this is a live bug at the tenth release, not a theoretical one. Guard a malformed `expiry_release` rather than letting `tonumber` abort the selector.

> **The helper is TWO gates, not one, and only one of them is mechanical — corrected 2026-08-02.** The Steps line calls this "a checked fact, not a judgment", but the row has two arms and **`replacement_trigger` is a free-text string** (`registries.sh:24` writes whatever `oss fake_add`'s `$4` was — e.g. *"when the vendor ships a sandbox"*). No jq selector can evaluate whether that has *fired*. Split them explicitly:
> - **Mechanical arm — `oss_reg_expired_fakes`:** the outstanding-and-at-or-before-this-release set. A checked fact; the close blocks on a non-empty return.
> - **Judgment arm — prose in `fake-expiry.md`:** for each *remaining* outstanding fake, surface its `replacement_trigger` verbatim and ask whether it has fired. Do **not** pretend a selector decides this, and do **not** silently drop the arm — spec §6.1 names it.
...
4. **Blocking finding — quarantine:** every line quarantined in a release **before** this one must now be fixed or retired. A parking ticket, not a shrug. The record carries its release (`ledger.sh:101-107` writes `{id,status,by,reason,release}`), so the set is `.demo_ledger[] | select(.status == "quarantined")` filtered to a release **numerically earlier** than this one — same `ltrimstr("r") | tonumber` rule as the fake gate, and wrong lexicographically at `r10` for the same reason. **This arm is mechanical too and deserves the same treatment as the fake gate** — either a sibling helper in `ledger.sh` (next to `oss_ledger_active_auto`) or an explicit jq in the prose. Do not leave one blocking finding mechanized and its twin a judgment.
5. **Release retrospective** (aggregates spine retros; refuses if any spine lacks one, naming it). The spine retros are the files T10's ceremony authors — resolve each spine's directory via `oss spine_dir` (**relative**; prefix with `oss repo_root ai_workspace`) and check for the retro T10's `retrospective.md` names. State the filename explicitly; a refusal gate that cannot find the artifact it gates on refuses everything.
6. **Feature-map re-groom + next-release sketch** — the rolling-wave crank. `oss feature_list` reads the map; the sketch lands via `oss release_set_meta <rel> '{"next_sketch": "…"}'` (one of the five allowlisted patch keys).
7. `oss release_status <rel> closed`; `oss demo_record release <rel> <passed> <line-count> "<notes>"` — **the elision hid three required arguments** (`oss_demo_record_close` is `$2=scope $3=id $4=passed $5=line-count $6=notes`), and T10's equivalent row was corrected the same way.
...
**Files:** Modify `skills/close/SKILL.md` (§6, **plus three new `references/` pointers**); create `references/{release-close,fake-expiry,patch-lane}.md`; **modify `ossify/lib/registries.sh`** (the new helper) **and `ossify/lib/commands.sh`** (its dispatcher wrapper); extend `tests/test-close.sh`.

> **Files list corrected 2026-08-02 — it contradicted this task's own Steps line.** It named zero lib files while the Steps line adds `oss_reg_expired_fakes` to `registries.sh` *and* a dispatcher wrapper (which lives in `commands.sh`). That is the same defect class as Task 7's `agents`-key contradiction (fixed in `07215a0`): **a Files list that disagrees with the Steps points the implementer and the reviewer the same wrong way.** `close/SKILL.md` is 292 lines at HEAD; T11 should land **≤~440**, leaving T12 its harvest pointer under the ~500 cap.

- [ ] **Step 1: Author the three references.** `release-close.md` (the 7-step binding order above, with the C2 deferrals stated rather than silently missing); `fake-expiry.md` (**both arms** — the mechanical selector and the judgment pass over `replacement_trigger`, each labelled as such); `patch-lane.md` (the routing judgment; the verb already exists).

- [ ] **Step 2: Add the mechanical helper** `oss_reg_expired_fakes <state> <release>` in `registries.sh`, plus a dispatcher wrapper in `commands.sh`. **Pin a return contract** the way every sibling gate does (`oss_reg_touch_check` is 0=hit / 1=clean / 2=inconclusive; `oss_verify_report_cross_check` is rc 1 + a named message): emit one line per blocking fake on stdout and **return rc 0 when the blocking set is EMPTY, rc 1 when it is non-empty** — or the inverse, but *state which*, because "returning the blocking set" specifies neither the rc nor the shape and the ceremony branches on it. Selector: `status` is `active` or `renewed` **and** the expiry compares **at-or-before, numerically** (see the boxed note above). Strict mode: guard the no-match case and a malformed `expiry_release` so `tonumber` cannot abort the selector.

- [ ] **Step 3: Test — and the fixture set is the point.** Extend `tests/test-close.sh` (its workspace fixture already builds a real git repo + `pairing.json` + `oss init`; it has **no** fake coverage today). At the close of release **R**, assert:
  - **(a)** an `active` fake with `expiry_release == R` **blocks**;
  - **(b)** a **`renewed`** fake with `expiry_release == R` **also blocks** — *this is the discriminating fixture*;
  - **(c)** an outstanding fake with `expiry_release` **earlier than R** (e.g. `r1` at `r3`'s close) **still blocks** — the at-or-before arm;
  - **(d)** a fake renewed to a **later** expiry does **not** block;
  - **(e)** a `replaced` fake at `expiry_release == R` does **not** block (the resolving-status control);
  - **(f)** an `r2` expiry at `r10`'s close **blocks** — the numeric-vs-lexicographic guard;
  - **(g)** a quarantine from an earlier release blocks.

> **THE TEST CLAUSE WAS THE DEFECT — corrected 2026-08-02, and this one is subtle.** It read: *"assert an unresolved expired fake blocks the close **and a renewed one does not**"*, which contradicts this task's own boxed note at the top (*"T11's test must seed a **renewed** fake at its expiry release and assert the close is blocked"*) and contradicts binding-order step 3, which puts `renewed` inside the selector.
>
> **Worse than a contradiction: the fixture pair it named is green under BOTH the correct and the defective selector.** Verified by encoding both selectors over four fixtures at closing release `r1` — `active@R`, `renewed@R`, `renewed@later`, `replaced@R`. The correct selector returns `[active@R, renewed@R]`; the defective `active`-only selector returns `[active@R]`. The two cases the old clause named are **identical under both**. So a test written faithfully from the checkbox **passes with the broken helper** — the literal *"passes either way and proves nothing"* outcome the boxed note was added to prevent, reproduced in the very clause meant to implement it. **(b) is the only fixture that discriminates, and the old clause never asked for it.**
>
> Note `replaced` is the *only* resolving status, so status and expiry are **both** load-bearing — do not simplify this to "expiry is the discriminator" and drop the (e) control.

- [ ] **Step 4: Wire the pointers, run the gates, commit.** Add **three** pointers to `close/SKILL.md` — one per new reference, matching the shipped one-pointer-per-reference convention (T9's `routing.md`/`work-item-close.md`/`impl-check.md`, T10's three at §5). `release-close.md` belongs in §6, currently a stub. Confirm `close/SKILL.md` ≤~440. Then `bash ossify/tests/run-all.sh` (ALL GREEN), the repo-root parity suite, and `bash ossify/tests/eval/lib/aggregate-scores.sh` (exit 0). Commit `ossify/skills/close ossify/lib/registries.sh ossify/lib/commands.sh ossify/tests/test-close.sh`.

**Named risks:** (1) **who calls this?** — `fake_status` and the new expiry helper must actually be driven here. (`fake_status` already has *prose* callers in `plan-spine`; what it has never had is a **close-time** caller, and the helper is new with none.) And the reverse: name the line where `oss_ledger_add_patch`, `oss_entity_set_release_status` and the release-scope `oss_demo_record_close` each gain a first non-test caller; record any that gains none. (2) Does the walkthrough use the **amended** active set? (3) Is the expiry finding genuinely blocking, with the only unblocks being replace or explicit renew — and is the comparison **at-or-before and numeric**? (4) Is the release-scope quarantine comparison right (quarantined in an *earlier* release, not this one), and is it computed **numerically** too? (5) Are **all three** C2 deferrals stated rather than silently missing — docs increment, **handoff cleanup**, PR gate? (6) Does the patch lane avoid inventing a new verb, and does it point at `oss touch_check` as the mechanical way to check "touches no bone / no risk surface" rather than leaving it a pure judgment? (7) Do **all three** references get a pointer from `close/SKILL.md` itself? (8) Does the helper have a **stated rc contract**, and does the test prove the discriminating fixture (b)?
...
**Files:** Create `ossify/lib/harvest.sh`, `ossify/tests/test-harvest.sh`, `skills/close/references/harvest.md`. Modify `ossify/lib/commands.sh`, **`skills/close/SKILL.md`** (the harvest pointer — Task 13's check 5 fails the suite on an unreferenced reference), **and `skills/close/references/spine-close.md`** (step 9 — see below).

> **`spine-close.md` added to the Files list 2026-08-05.** T12 claims its payload shape is "pinned here because the ceremony in **Task 10 step 9** authors it". **Task 10 shipped no such ceremony.** What it shipped is a bare one-line row — *"**Step 9 — memory-bank harvest, always before cleanup.**"* (`spine-close.md:315`) — naming **no verb, no payload, no shape**; `grep -n 'harvest_apply\|harvest_dir\|payload' spine-close.md` returns nothing. So the "pin" pins one side of a two-sided contract, and the caller T12's named risk 1 relies on does not actually call anything. T12 must fill that row in: name `oss harvest_dir` / `oss harvest_apply`, the payload shape, and the one-call rule. `close/SKILL.md` is **354** lines at HEAD against a ~500 cap; T12 is the last writer, so land **≤~420**.

**Interfaces:**
- Produces (usage shapes below are written in `text` fences, never ```` ```bash ````, because `<placeholder>` parses as a **redirect** under `bash -n` — three shipped blocks already fail Task 13's check 1 for exactly this):

  ```text
  oss_harvest_memory_bank_dir                      # NO state argument - see below
  oss_harvest_apply <state> <payload-json>
  ```

  `oss_harvest_memory_bank_dir` echoes the memory-bank directory, honouring `.well_known_paths.memory_bank` with an `<ai_workspace.root>/.claude/memory-bank` convention fallback; **rc 1 if the resolved path still contains an unexpanded `${…}` token**, and rc 1 if unresolvable. `oss_harvest_apply` — rc 0 on any write **or on an empty payload**, rc 1 if the payload was non-empty and nothing was written, **rc 2 if the payload is rejected**; echoes `harvest: wrote <N>, skipped <M>` on stdout. Dispatcher: `oss harvest_dir`, `oss harvest_apply`.

> **`oss_harvest_memory_bank_dir` takes NO state argument — corrected 2026-08-05.** The house rule is *"all **state** functions take the state-file path first"*, and this is not one: the manifest is found by `oss_manifest_discover` walking up from `$PWD`, never from the state file, and the function reads no state. Its true sibling is `_oss_repo_root` (`worktree.sh:11`), which takes a repo key, not state. A vestigial `<state>` parameter invites a caller to pass one and a reviewer to "fix" a function that ignores it.
>
> **And "rc 0 on a clean no-op" collided with "rc 1 if the payload was non-empty and nothing was written":** eight items that are all duplicates write nothing and *are* non-empty. Named risk 4 asks exactly this. Resolved above — rc 0 covers writes and the empty payload; an all-duplicates payload is rc 1 with `wrote 0, skipped 8`, which is the honest signal.
...
- Consumes: `oss_manifest_discover` **and `_oss_manifest_resolve`** (`manifest.sh:49`) for the memory-bank path — **not `oss_manifest_get` alone.**

> ### The token-resolution P0 — this sweep's find (2026-08-05)
>
> `oss_manifest_get` does **no token expansion**; it is a bare `jq -r "${expr} // empty"` (`manifest.sh:33-39`). And `.well_known_paths.memory_bank` is **token-bearing on every manifest workspace-init emits** — `workspace-init/lib/manifest.sh:328` writes `"${ai_workspace.root}/.claude/memory-bank"` literally. So the prescribed mechanism returns the *string* `${ai_workspace.root}/.claude/memory-bank` on essentially every real project.
>
> **Reproduced end to end.** With that manifest, `oss manifest_get '.well_known_paths.memory_bank'` returns the literal token string; `mkdir -p "$dir" && printf … >> "$dir/09-known-issues.md"` then creates a directory **literally named `${ai_workspace.root}`** under `$PWD`, writes the entry into it, and the harvest reports **`harvest: wrote 1, skipped 0`, rc 0** — while `<ai_workspace>/.claude/memory-bank` is never created at all.
>
> **T12's own safety net cannot catch this.** The count contract only returns rc 1 when *nothing* was written; here everything was "written", just nowhere. That is precisely the *"rc 0 and nothing where you are looking"* signature this task exists to eliminate (line 2290 says so in as many words) — **reintroduced by the mechanism specified to implement it.**
>
> **The fix already exists in the same file.** `oss_manifest_state_path` (`manifest.sh:66-84`) reads `.ai_workspace.root`, pre-expands `${HOME}`/`${USER}`, routes the raw value through `_oss_manifest_resolve`, and then guards `case "$dest" in ''|*'${'*) … return 1`. Mirror it exactly. The port source's own sibling already does this (`scaffold-dev/lib/state.sh:40-47`); `scaffold-dev/lib/harvest.sh:145-146`, which hardcodes the path, is the one that does not — that is the second half of D8.

- [ ] **Step 1: Write the failing test** covering exactly the defect class. Build the fixture the way `tests/test-manifest.sh` does — **a real `pairing.json` rooted so the walk-up from the test's cwd finds it** (`oss_manifest_discover` starts at `$PWD` and `run-all.sh` does not `cd`, so tests inherit the repo root). Fixtures:
  - **(a)** text containing a blank line;
  - **(b)** multi-line text with one line already present;
  - **(c)** text that is a substring of existing content;
  - **(d)** text starting with `-` — **applied TWICE** (see below);
  - **(e)** the same payload applied twice, carrying the (a)-(d) texts;
  - **(f)** a manifest whose `memory_bank` is the **token form** `${ai_workspace.root}/.claude/memory-bank`, asserting the function echoes the **resolved absolute** path — mirror `test-manifest.sh:33`, which already uses the token form for `project_state`;
  - **(g)** a manifest with an **unresolvable** token (e.g. `${private_core.root}/mb`), asserting **rc 1**.

  Assert the written/skipped **counts** and the resolved path, not just rc.

> **Two of these fixtures were vacuous as first written — corrected 2026-08-05, both verified by execution.**
>
> **(d) cannot discriminate on a single apply.** The leading-dash defect is that `grep -Fq "-foo" file` treats `-foo` as options — it exits **rc 2 with an error** (`grep: oo: No such file or directory`), not rc 0. rc 2 makes the `if` false, so the code appends. But on a *first* apply the correct implementation also appends, because the text genuinely is not there yet. **Both implementations append, and the fixture proves nothing.** It only discriminates on the **second** apply, where the correct one skips and the broken one appends again. Step 4's mutation gate names this fixture, so without the second apply that gate cannot go RED.
>
> **(f)/(g) are new and load-bearing.** A literal-path manifest fixture passes with the token-blind implementation and proves nothing — see the token-resolution box above.
>
> Also note the plan's own account of the leading-dash mechanism ("consumed as an option and the append fires unconditionally") is right about the *outcome* and loose about the *mechanism*: it errors at rc 2 rather than silently misparsing. The fix is the same — `grep -F -- "$pattern"`.
...
- **Idempotency is an exact-entry match on a content hash carried in the provenance trailer**, never a `grep -F` over the text: `h="$(printf '%s' "$text" | cksum | awk '{print $1}')"`, trailer `<!-- ossify harvest: <source-id>, <date>; source: report|handoff; h:<hash> -->`, and the skip test is **`grep -Fq -- "h:${h} -->" "$mb/$target"`** — anchored on the trailer's closing delimiter, `-F` so the hash is literal, `--` so no argument can be read as an option, and **scoped to the target file** so a hash in `09-known-issues.md` cannot suppress a write to `10-decisions-log.md`. A hash has no blank lines, no leading dash, and no substring ambiguity — the entire defect class is designed out rather than patched.

> **The unanchored skip test reintroduced substring ambiguity — corrected 2026-08-05.** `grep -q "h:${h}"` is a **prefix match**: `cksum` emits variable-width decimals, so a stored `h:1234` satisfies a lookup for `h:123` and the new item is silently skipped — the same "skips when it should write" outcome as the `grep -F` bug the hash was introduced to kill, arriving through a narrower door. Anchoring on the ` -->` the trailer already ends with costs nothing structurally. The plan's claim that a hash has "no substring ambiguity" is true of the *hash* and false of the *unanchored lookup*.
...
- [ ] **Step 3: Author `harvest.md`** — the ceremony. **Enumerate first, and say how:** the spine's work items are `oss get '[.work_items[] | select(.spine == "<spine-id>")]'`, and each `report.md` sits beside its `spec.md` in the work item's docs directory under `oss spine_dir` (**relative** — prefix with `oss repo_root ai_workspace`), exactly as T9's work-item close layer resolves it. Reuse that resolution rather than restating it; a ceremony that cannot find its inputs refuses everything.

  **The `[handoff]` half is C1-live but narrow, and the prose must say which handoff it means:** `/handoff` (session handoffs) is Plan C2 and C1 ships none, so the only handoffs that exist are the **per-work-item `handoff.md` T8 authors**. Scope the read to those and say so, or the `source: "handoff"` enum arm is dead in C1 and the tag is theatre.

  Then: read every work item's `report.md` §9 and every in-scope handoff; extract candidates; categorize (caveats → `09-known-issues.md`, decisions → `10-decisions-log.md`; an *enforceable* pattern is never a raw append — it is a C2 rule-authoring referral, and say that it is deferred); surface a numbered list where **every first line starts with the literal `[report]` or `[handoff]`** (the tag is a trust-calibration signal: report-origin is grounded in just-written code, handoff-origin in conversational context); consume accept/edit/reject; apply the accepted array in **one** call; record outcomes distinguishing applied / applied-with-edit / left-in-handoff / dropped.

  > **Where those outcomes are recorded — corrected 2026-08-05.** The text said "record outcomes **in the retrospective**", but T10's binding order authors the retrospective at **step 8** and runs the harvest at **step 9**: the retro is already written when the harvest starts, and nothing says to reopen it. Either the harvest **appends a section to the retro it just followed** — say that explicitly, since it is an edit to a completed artifact — or the outcomes land in the harvest's own return and the close summary. Pick one and name it; as written the ceremony records into a document that no longer accepts writes. **Harvest never removes handoffs**, and it runs **before** worktree cleanup — but **not** because it reads `report.md` out of the worktree. It does not: `report.md` lives beside `spec.md` and `handoff.md` in the work item's docs directory (`work-item/SKILL.md:253`), which worktree removal never touches. The ordering holds because cleanup is terminal — `oss_worktree_remove` refuses an unmerged branch rc 8 and, once it succeeds, the branch is gone — so every step that might still need to halt must run first. *(Corrected 2026-08-01: the same false clause was corrected in Task 9; this was its second copy.)*

- [ ] **Step 4: Mutate, gate, commit.** Mutation-test by replacing the anchored hash match with the source's `grep -Fq "$text"` and confirm the **blank-line** fixture and the **twice-applied leading-dash** fixture both go RED (the latter only discriminates on the second apply — see Step 1). Add a second mutation: **drop the `_oss_manifest_resolve` call** and confirm the token-form fixture (f) goes RED — the count/rc contract alone cannot catch it, because a write into `./${ai_workspace.root}/…` still reports `wrote N` at rc 0.

  **Run the mutations under `bash`, not your interactive shell.** The blank-line defect depends on `grep -F` treating an empty pattern line as a match-everything alternative, which **BSD grep does and `ugrep` does not**. `bin/oss` and `run-all.sh` both force `bash`, where bare `grep` is `/usr/bin/grep` (BSD) and the defect reproduces; a zsh session with `ugrep` on the PATH shows the opposite and would read as "the plan is wrong". Verified both ways.

  Then the gates: `bash ossify/tests/run-all.sh` (ALL GREEN), the repo-root parity suite, and `bash ossify/tests/eval/lib/aggregate-scores.sh` (exit 0). Commit `ossify/lib/harvest.sh ossify/lib/commands.sh ossify/tests/test-harvest.sh ossify/skills/close`.

**Named risks:** (1) **who calls this?** — **T10 step 9** must drive it (binding-order item 9 of the spine-close checklist T10 authors into `references/spine-close.md`: *"Harvest (T12) — before cleanup, always"*); confirm the ceremony and the lib agree on the payload shape. *(Corrected 2026-08-01 — this said "T9 step 9", which does not exist: T9's work-item close layer has six steps and no harvest row.)* (2) Does the blank-line fixture genuinely reproduce the source's bug before the fix? Prove the RED. (3) Is the whole-payload rejection genuinely before any write? (4) Does the count-return distinguish "wrote 0 because all duplicates" from "wrote 0 because rejected"? (5) Does the manifest key take precedence over the convention path? (6) Does §9's heading in T7's report contract match what this greps for — exactly?
...
**Files:** Create `ossify/tests/test-skill-bash-blocks.sh`. **Modify** `ossify/skills/plan-spine/references/{demo-amendments,fake-ledger-discipline}.md` (the three check-1 findings — see the adjudication below) and `ossify/tests/test-close.sh` (drop its now-superseded orphan loop, `:145-152`).

> **Files list corrected 2026-08-05 — it named one file while the Steps line says "fix whatever it legitimately finds".** A harness task that fixes findings touches the files holding them; listing only the harness is the Files-vs-Steps contradiction this plan has now hit four times (T7's `agents` key, T11's helper, T12's `spine-close.md`, here).

> ### The adjudication the plan never made: fix the CONTENT, scope nothing
>
> Step (line 2371) says "expect real findings … fix whatever it legitimately finds" but never says which HEAD failures are **content to fix** and which are **checks to scope**. That decision is the whole task — a harness whose checks get weakened until the suite goes green is worth nothing, and Task 12's precedent of re-fencing usage shapes to ```` ```text ```` actively pulls toward the wrong answer here. Decide it up front:
>
> **Check 1 — 3 failures of 141 blocks. Fix the content by QUOTING the placeholders.** All three are usage signatures with *unquoted* `<placeholder>` tokens (`demo-amendments.md:12`, `:97`, `fake-ledger-discipline.md:57`): `<line-id>` parses as a redirect, `<active|replaced|renewed>` as a syntax error near `|`. The parse-clean convention is **already dominant in this tree** — 87 blocks carry quoted `<...>` placeholders and parse fine (`release-close.md:193`, `cumulative-demo.md:101`, `fake-expiry.md:152-153`). The three failures are the deviants, not a new class. Verified: all three re-quote to parse-clean forms.
>
> **Do NOT add a skip-if-`<...>` heuristic.** Measured: it silences check 1 on **87 of the 138 currently-parsing blocks (63%)**, including the sole bash block in *all five* command files, and it demonstrably swallows real errors — a block with a quoted placeholder plus an unterminated `if`, an unclosed `for`, or an unterminated quote is skipped while `bash -n` calls it an error (3 of 4 scratch cases).
>
> **Do NOT re-fence these to ```` ```text ```` (Task 12's move was right for Task 12 and wrong here).** Quoting keeps the blocks inside check 1's coverage permanently; `text` removes them, *and* hides their `oss` verbs from check 2 — which is the check these very lines exist to feed. Reserve `text` for shapes that cannot be made parse-clean by quoting, and require an inline justification so the exception cannot become the default.
>
> **Check 4 — 5 failures, and all five are FALSE POSITIVES: scope the check.** Every command file carries, at line 7, the literal warning *inside backticks*: ``Parse args from `$ARGUMENTS` via the env-var bridge (no positional `$1`/`$2`/`$N`)``. There is **not one real positional** in `ossify/commands/`. A check that flags prose telling you not to do the thing is broken, not the prose. Strip inline-code spans before matching — and say explicitly that this creates a blind spot for a real positional written inside backticks as example code, which is accepted because the bridge line is a shipped convention in all five files and a backticked positional is not a form anyone writes by accident.

- [ ] **Checks, each failing with the offending file, line and token:**
1. Every ```` ```bash ```` block in `ossify/skills/**` and `ossify/commands/**` parses under `bash -n`. **Pin the extraction contract:** **both** fence regexes must tolerate leading whitespace — opener `^[ \t]*```bash[ \t]*$`, closer `^[ \t]*```[ \t]*$` — because **8 of the 141 blocks are indented inside list items**, and an anchored `^```bash$` silently skips them. The corpus at HEAD is **141 blocks, 138 parsing, 3 failing**; assert the *extracted count* as well as the failure count, so a regression in the extractor cannot masquerade as a clean run. The tree also contains ```` ```text ````, ```` ```markdown ````, ```` ```json ```` and one ```` ```jq ```` fence — none are bash and none are in scope for this check.
2. Every `oss <subcommand>` token appearing in any block or inline-code span resolves to a real `oss_cmd_*` in the dispatcher. **State the extraction rule, because the naive one has 7 false positives at HEAD:** match `oss` only at a word boundary (`\boss ` — otherwise the `oss` in "acr**oss**" yields `oss a`, `oss all`, `oss every`, `oss the`, `oss three`), and treat a token ending in `_` or containing `*` as a deliberate wildcard mention rather than a verb (`oss ledger_add_`). Report every excluded token so the exclusions stay visible.
3. No `Skill(` invocation carries an argument — match `Skill\([^)]*,` across `ossify/skills/**`, `ossify/commands/**` **and `ossify/agents/**`**. Zero hits at HEAD. Scoped to ossify only: `scaffold-onboard` still legitimately ships the v0.1.x parameterized form and is not this harness's business.
4. No positional `$1` / `$2` / `$N` in `commands/*.md`, **matched only outside inline-code spans** — see the adjudication above; all five files carry the pattern inside backticks as a warning and contain zero real positionals.
5. **Orphans:** every `references/*.md` under a skill is named by that skill's own `SKILL.md`. Clean at HEAD (43/43). **This supersedes the per-skill loop `tests/test-close.sh:145-152`** — delete that loop when this lands rather than leaving two enforcers of one rule. **Dangling:** state the resolution base explicitly — a pointer written `references/foo.md` resolves relative to the **skill directory**, not the repo root or the citing file's directory; enumerate the pointer forms actually present before implementing, and skip `${CLAUDE_PLUGIN_ROOT}`-rooted pointers, which are runtime-expanded and not resolvable at test time.
6. Every SKILL.md is ≤500 lines **measured as whole-file `wc -l`** (the shipped budgets — `plan-spine` and `start` at exactly 499 — are whole-file counts, and the Global Constraints' "≤~500" is enforced that way throughout this plan), reported with its current count so the headroom is visible before an edit hits the ceiling. Two files sit at 499, so treat this as a **hard** fail, not a warning: there is no headroom left to absorb a rounding argument.

- [ ] **Steps:** write the harness; run it against HEAD and confirm it reports **exactly the findings adjudicated above** — 3 for check 1, 5 false positives for check 4 (which the scoping removes), 0 for checks 2/3/5/6; apply the content fixes (quote the three signatures) and the check scoping; delete the superseded `test-close.sh` orphan loop; commit `ossify/tests/test-skill-bash-blocks.sh ossify/tests/test-close.sh ossify/skills/plan-spine/references`.

  **`run-all.sh` needs no edit** — it globs `"$HERE"/test-*.sh` (`run-all.sh:5`), so the file is picked up by name alone. The plan's "add it to run-all.sh's glob" implied an action there is none; the only obligation is the filename prefix.

- [ ] **Build in a PERMANENT self-test, not a one-time planted token.** Named risk 2 asks the author to plant a bad token *once* during authoring. That proves the extractor worked on the day it was written and nothing thereafter — and this artifact **is a test**, so a harness that silently stops extracting is invisible forever, and every check it owns goes quietly green. Ship a fixture directory (or heredoc-built scratch tree) carrying one known-bad instance per check, and assert the harness **finds exactly those**. That is the only construction under which a green run means anything. This is the vacuous-test problem this series has hit five times, in the one place where it can never be caught downstream.

**Named risks:** (1) Does check 2 handle multi-word and quoted invocations without false positives, **and does it exclude the 7 known non-verb tokens by rule rather than by allowlist?** (2) Does the harness find exactly the adjudicated finding set at HEAD — and does the permanent self-test prove the extractor is live, not just the codebase clean? (3) Does it catch the *historical* bugs? **Reconstruct each of the three from the repo before claiming coverage** — and note the honest answer for two of them, below. (4) Is it fast enough not to dominate `run-all.sh`? (5) Is the fix-vs-scope decision applied as adjudicated — content quoted, check 4 scoped — with **no check weakened to reach green**?

> **The Context's coverage claim is overstated on two of its three bugs — corrected 2026-08-05.** Check 2 as worded resolves `oss <subcommand>` tokens, i.e. ossify's own dispatcher verbs. **`sd_rules_apply` is a lib-function name, not an `oss` subcommand**, so the check named as "the check that catches the `sd_rules_apply` class" would not catch that literal instance — it catches the *analogous* class *within ossify's verb surface*, which is the honest claim. (The underlying defect is real and still live: 5 citations, 0 definitions.) Likewise **B8's `[internal]` marker break is a cross-skill semantic contract, not a parse or symbol fact** — no mechanical check here catches it, and named risk 3 asking for a RED on it cannot be discharged. Say both plainly rather than letting the harness inherit credit for coverage it does not have; a test justified by bugs it cannot catch is how the next `sd_rules_apply` passes an eval.
...
**Files:** Create `ossify/tests/test-integration-close.sh`, `ossify/tests/eval/fixtures/close-gate-integrity/{01..05}-*.md`, `ossify/tests/eval/rubrics/close-gate-integrity.md`. Modify `ossify/tests/eval/RUNBOOK.md` (register the surface) and `ossify/tests/eval/lib/aggregate-scores.sh` (the unscored-fixture guard).

> **Files list added 2026-08-05 — Task 14 was the only build task in this plan without one**, and two of the four entries above are files the Steps never named at all (RUNBOOK, aggregate-scores).

- [ ] **Step 1: Integration test** — `ossify/tests/test-integration-close.sh`, driving the whole arc through `bin/oss` under real strict mode: init → release → spine → work items → exec fields → demo lines → a planned amendment → work-item close → apply-pending → spine close → **a second spine, demo-recorded and closed** → release close with an expired fake → resolve it → release closed. Assert replay stays clean at the end and that `close_records` holds **one record per SPINE and RELEASE close — two, not three.** Scope the header honestly to what it actually exercises and name where the un-exercised ops are covered — the B9 overclaim lesson.

> **Three corrections to the arc, 2026-08-05, each reproduced by execution.**
>
> **(a) The arc as written is ILLEGAL and would go green anyway.** It adds a second spine and then closes the release, but `release-close.md` §2 halts on any spine that is neither `closed` nor `abandoned` — executed: `open_spines` returns `r0.s2 (planned)` and the gate prints *"has spines that are not closed … halt"*. **And `oss release_status` enforces nothing** (`entities.sh:85-93` validates only the enum), so an implementer who omits the §2 gate from the test writes `oss release_status r0 closed`, the suite goes **GREEN**, and the plan's own ship-gate integration test asserts a close the shipped ceremony would refuse. That is exactly the B9 overclaim failure mode, in the test built to prevent it. Close (or abandon) the second spine first, and have the test **run §2's selector and assert it is empty** before the fake gate.
>
> **(b) "the de-facto full-op replay guard" is an overclaim — in the sentence that invokes the B9 lesson.** The arc journals **13 of the 26 `_oss_apply_op` cases**, exactly half. Untouched: `set_posture`, `set_composition`, `set_overlay`, `set_spine_class`, `add_bone`, `add_risk_gate`, `add_feature`, `set_demo_line_status`, `clear_demo_pending`, `add_patch_record`, `set_release_meta`, `add_veto_disposition`, `migrate_schema`. Say "a replay guard over the 13 ops this arc journals" and name where the other 13 are covered.
>
> **(c) `close_records` holds TWO records, not three.** The arc names three closes, but **the work-item close layer writes no close record** — `work-item-close.md` contains no `demo_record` call, and only `spine-close.md:356` and `release-close.md:294` write one. That is deliberate (the plan's own type-consistency note says C1 records only `spine` and `release`), so an assertion of "one per close" over three closes fails on correct behaviour.

- [ ] **Step 2: New eval surface `close-gate-integrity`** — 5 fixtures under `tests/eval/fixtures/close-gate-integrity/`, plus **one rubric FILE at `tests/eval/rubrics/close-gate-integrity.md`** (rubrics are one `.md` per surface, not a directory — the brace expansion in the original wording implied a `rubrics/close-gate-integrity/` directory that matches nothing in the tree). Follow the established protocol (**invoke agent receives the fixture BODY ONLY, frontmatter stripped — the frontmatter is the answer key; the judge sees the full fixture**; run as a Workflow by the controller, never inline by anyone who has read the keys):

> **Registering the surface is a real edit, not a consequence — corrected 2026-08-05.** `RUNBOOK.md:7` hardcodes the surface list (`For each surface in [posture-derivation, journey-line-floor, spine-class-declaration, bone-touch-check, critic-veto-interpretation]`). Authoring fixtures and a rubric **does not add the surface to any run** — RUNBOOK.md must be edited too.
>
> **And the gate cannot detect its own incompleteness.** `aggregate-scores.sh` counts result JSONs and never looks at `fixtures/` (`grep -c fixtures` → 0). If the new surface is authored but never *run*, it reports **23/23 and exits 0** — a green ship gate over a surface that does not exist in the results. Step 5's "28/28" is therefore not self-enforcing. **Add an unscored-fixture guard to `aggregate-scores.sh`**: walk `fixtures/*/` and fail if any fixture has no corresponding `results/<surface>/<id>.json`. Key it off fixtures, which are authoritative, not results.
>
> **Fixture frontmatter:** match the shipped shape — `scenario_id` plus an expectation key (`expected_verdict` in `bone-touch-check`); read one existing fixture's keys before authoring and keep the set consistent.
>
> **The criteria are per-SURFACE and every criterion is scored on every fixture** — that is the established convention, and it makes the negative control well-posed rather than needing an N/A mechanism: on a clean flesh spine, `halt_correct`/`reclassification_correct` are scored on whether the skill correctly **declines** to fire them. Precedent: `bone-touch-check`'s `02-clean` fixture scores 5 on `risk_gate` for exactly that reason. Do **not** invent an N/A convention; no rubric in the tree has one.
>
> **Who may invoke:** whoever authors the fixtures has read the answer keys and is disqualified as the invoke agent — that includes the implementer. **Sequencing: the implementer authors fixtures + rubric + RUNBOOK + the guard and stops; the controller then runs the eval as a Workflow with fresh invoke and judge agents per fixture.** Named risk 1 says this of the controller; it binds the author too.
...
- [ ] **Step 4: Budget re-measure — and the trim contingency FIRES; it is not hypothetical.** Pin the method so the comparison is like-for-like: **total the `description:` frontmatter field of every `skills/*/SKILL.md`, in characters, and estimate tokens as chars ÷ 4** (that basis reproduces the 2026-07-26 baseline exactly: 2,474 ÷ 4 = 619, and 619 ÷ 200,000 = 0.31%).

  **Measured 2026-08-05 with all five entry skills:** `close` 906 + `plan-release` 749 + `plan-spine` 899 + `start` 768 + `work-item` 858 = **4,180 chars ≈ 1,045 est. tokens ≈ 0.52% of a 200k window** — against §9.1's ≈0.3-0.4% target. **That is an overshoot, so trim descriptions before shipping**, exactly as this step already commits to. Re-measure after trimming and record the final figure **with its window**.

> **The "0.76% on the 1M model" claim is arithmetically impossible — corrected 2026-08-05.** The same listing measured against a *larger* window must give a *smaller* percentage: 619 tokens is 0.31% of 200k and **0.06%** of 1M, not 0.76%. Whatever 0.76% measured, it was not this listing on a 1M window — most likely a `/doctor` readout of the *whole* skill listing across every installed plugin, which is a different quantity with a different denominator. Do not carry the number forward without re-deriving what it counted. The defensible statement is the one this step now makes: ossify's own entry-skill listing, chars ÷ 4, against a named window.
>
> Verify `skillListingBudgetFraction` before repeating the claim that the stack is inside budget "because of the raised" setting — locate it, quote its value, and say which config it lives in. An unverified budget claim in the plan's closing task is the last place to leave one.

- [ ] **Step 5: Full green + close the plan.** `bash ossify/tests/run-all.sh` (ALL GREEN) · the repo-root parity suite (clean) · `bash ossify/tests/eval/lib/aggregate-scores.sh` (**28/28**, exit 0 — and with the unscored-fixture guard in place, so 28 cannot be reached by 23 results plus five unrun fixtures). Commit, then write the Plan C1 ledger's closing block.

  **The closing block must discharge, not drop, the deferred backlog.** Tasks 1-13 recorded a running set of `MINOR-FOR-FINAL` items in `.superpowers/sdd/progress.md` — among them `oss_cmd_manifest_get` having no named consumer, `oss zero_tests_guard` blocking on a bare invocation, `oss_worktree_resolve`/`oss_worktree_list` ending C1 with **no consumer at all**, `oss_harvest_apply`'s vestigial `<state>` argument, and the two-deep Task-13 scoping debt. Task 14 is the last task: **enumerate every one, and for each record disposition — fixed, wontfix with reason, or carried to Plan C2/D with the carrying task named.** An item that simply stops being mentioned at close has been dropped, not resolved.

  Also re-run the plan's **zero-consumer audit** against the final tree and correct it in place. It has been wrong in every sweep of this series, and the closing commit is the last chance to leave it true.
...
**Type consistency:** `oss_id_work_item_branch` (T3) is called by `oss_worktree_add` (T4, `worktree.sh:35`) and by its own dispatcher wrapper `oss_cmd_work_item_branch` — names match. **T9's merge does NOT call it** (this line said it did): it reads `work_items[].branch` back from state, the field `work_item_exec` wrote, because `close` holds an id and no slug. *Corrected 2026-08-01 — the Task 8 sweep's `$slug` fix changed T9's merge and left this audit line describing the pre-fix behaviour.* `oss_verify_zero_tests_guard` (T5) is consumed by `oss_demo_run_auto` (T6) — signature is `(command)` with output on stdin in both. `apply_demo_pending` (T2) is driven by T10 step 3. `oss_id_parse` (T3's new wrapper) is what T9's router calls. The report `## 9. Suggestions for memory bank` heading (T7) is exactly what T12's harvest greps, and T12's payload `target_file` allowlist matches the two files T10 step 9's ceremony proposes. **`demo_record_close`'s scope enum is `work_item|spine|release` (T6) but C1 only ever records `spine` and `release`** — T9's work-item close does not write one. That is deliberate (a per-work-item close record buys nothing the spine record does not carry), and T14's integration assertion is scoped to spine and release closes accordingly.
