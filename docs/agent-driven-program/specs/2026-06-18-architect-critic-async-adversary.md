...
| **Liveness** | `ac_codex_status` — one-shot non-mutating status; `ac_codex_wait` — bounded poll on `status`; stall heuristic; cancel on stall/cap; normalize `done`→`completed`; **always rc=0** | — |
