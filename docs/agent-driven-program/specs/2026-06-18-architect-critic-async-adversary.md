...
| **Liveness** | `ac_codex_wait` — bounded poll on `status`; stall heuristic; `cancel` on stall/cap; normalize `done`→`completed`; **always rc=0** | — |
| **Read return** | `ac_codex_result` — extract the last fenced ```json block; validate `.challenges` is an array | judge a `failed`/empty return; decide re-dispatch-once vs abort |
...
The companion runs a **bare prompt-file and never sees the skill/system prompt** (`feedback_codex_companion_no_auto_load_contract`). So the dispatch prompt-file = the existing adversarial prompt **+ an appended `## Return contract`** instructing Codex to end its turn with a fenced ```json `{"challenges":[{"text","severity","rationale"}], "gaps":[…]}` ``` block — the **exact** shape the sync path's `--output-schema` enforces and `consolidator.sh` already reads. `ac_codex_result` extracts the last fenced JSON block from `storedJob.result.rawOutput` (leading reasoning prose tolerated) and validates `.challenges` is an array via the (extended) `_ac_codex_validate_json`. No parseable block → re-dispatch-once → abort to claude-only (the run is recorded `failed`, never silently dropped).
...
4. Run the **shared** sequential rebuttal cycle + `ac_state_append_run` (critiquing-spec Steps 7–9, invoked identically to the sync path), then set `resolved_run_request_id` on the external run.
...
- **New `managing-async-critique` skill + `/critique-jobs` command** — owns `status [id]`, `result [id]`, `cancel [id]`, `resume [id]`. `resume` re-enters critiquing-spec Steps 7–9 (§3.5); `status`/`result`/`cancel` are thin spine/state reads + `ac_codex_cancel`.
...
- **State v3:** migration v2→v3 idempotent + preserves all v2 fields + no `in_flight`; `external_runs` CRUD lock-guarded; cap honored; `resolved_run_request_id` set-once.
- **Resume semantics:** persisted-claude + fetched-codex → consolidate (cross-confirmation) → rebuttal → append; **re-resume = inspect-only** (no duplicate `recent_runs`); resume of a non-terminal job reports status and appends nothing.
