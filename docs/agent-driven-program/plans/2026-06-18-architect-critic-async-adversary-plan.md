...
- [ ] **Step 3: Port the spine into `lib/codex.sh`** — at the END of the file (after the existing sync functions), add an async section. Port these functions from `scaffold-onboard/lib/codex.sh` with the transforms below. **Read the source file first**, then apply uniformly:
  ...
  - `ac_codex_dispatch`: keep the `task --background --prompt-file <abs> --json` launch + jobId extraction, but **drop `--write`**. The async adversary is read-only by contract; if the companion rejects that mode, hard-fail with remediation rather than weakening isolation. Drop `--resume-last`/`--fresh` flags (not needed here).
