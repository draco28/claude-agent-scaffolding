...
### SS-8 — `/amend-spec`: incremental, change-driven MASTER-SPEC amendment (`#86`) · independent · scaffold-onboard-only · 🟢 design-locked 2026-06-26, building
...
| **#86** | scaffold-onboard: `/amend-spec` skill — incremental, **change-driven** MASTER-SPEC amendment for the post-MVP / vNext lifecycle (classify change → impact analysis vs MASTER-SPEC+SRS/BACKLOG → collision-safe ID mint from ledger → *targeted* spec edit + *diff-aware* governance-doc update, vs today's greenfield whole-bundle re-derive) | independent (enhancement; **larger design item** — brainstorm-first) | **SS-8** (own sub-spec) | 🟢 **design-locked 2026-06-26, building** (PR pending) — brainstorm reframed to a **front-door MVP**: agent-driven classify → impact analysis → *targeted* MASTER-SPEC edit + `## Revision History` + SSoT `phase_record` fold; **diff-aware doc-merge + stable ID minting deferred** (envelope choice — MASTER-SPEC is ID-free, `/scaffold-docs` re-mints wholesale), propagation routes to `/scaffold-docs` + `/plan-roadmap`. Zero new `lib/*.sh`. Design of record `specs/SS-8-amend-spec.md`; ships as `scaffold-onboard v0.12.0`. Demand-validated by PulseDB v0.5.x→vNext. |
