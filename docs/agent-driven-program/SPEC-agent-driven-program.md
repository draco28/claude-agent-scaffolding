...
| **#39** | architect-critic async adversary (+ doctor, state v3, scaffold-dev gate) | independent | **SS-6** | **Phase A ✅ SHIPPED 2026-06-18 (architect-critic v0.3.0)** — async lifecycle + readiness doctor + size guidance + state.json v3 `external_runs[]` + defer-to-resume unified rebuttal; also added architect-critic to CI + fixed a pre-existing macOS suppression-date bug. **Phase B** (scaffold-dev opt-in review gate #6) pending (own plan, scaffold-dev v0.8.0). Spec: `specs/2026-06-18-architect-critic-async-adversary.md` |
