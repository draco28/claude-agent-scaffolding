...
### SS-8 — `/amend-spec`: incremental, change-driven MASTER-SPEC amendment (`#86`) · independent · scaffold-onboard-only · ✅ SHIPPED 2026-06-27 (PR #91, squash `a3c8b9c`, tag `scaffold-onboard-v0.12.0`)
**Design settled with user 2026-06-26** (brainstorm; four forks locked via AskUserQuestion). The greenfield→full-lifecycle-tool gap: a **front-door MVP** `/amend-spec` skill (`amending-spec`) that folds **one** change (new capability / hardening NFR / scope tweak) into an existing `MASTER-SPEC.md` — classify → impact analysis → **targeted** phase-section edit + `## Revision History` + bumped `**Spec revision:**` (schema `**Spec version:**` pinned) → **SSoT fold** of the rationale into the affected `phase_record` (so `/onboard --regenerate` reconciles forward, not clobbers) → EXEC-SUMMARY judgment → propagation handoff. **Fully agent-driven, zero new `lib/*.sh`** (North Star §1): classification, impact analysis, and every edit are the agent's; bash is reused only for path/validation/state mechanics via `sf`. **Deferred by design** (the envelope): diff-aware governance-doc merge (the #58-class complexity, one layer down) and stable/collision-safe ID minting (`MASTER-SPEC.md` is ID-free; `/scaffold-docs` re-mints wholesale) — propagation routes to existing `/scaffold-docs` + `/plan-roadmap`. Design of record: `docs/agent-driven-program/specs/SS-8-amend-spec.md`. Demand-validated by PulseDB v0.5.x→vNext.
**Closes:** `#86`.

### SS-9 — `/work-pr`: standalone, slice-decoupled PR review-fix-merge gate (`#92`) · independent · scaffold-dev-only · ✅ SHIPPED 2026-06-30 (PR #94, squash `1b0eebb`, tag `scaffold-dev-v0.14.0`)
**Design settled with user 2026-06-27** (brainstorm; two forks locked via AskUserQuestion). The #82 multi-reviewer pre-merge gate was reachable **only** from slice/sprint close; this exposes it as a standalone `/work-pr <PR>` command (`working-pull-request` skill) for any PR — e.g. an ordinary feature PR like #91, which lived in a **manifest-less** repo. **Job boundary (fork 1):** the *full* review-fix-merge loop — fetch findings → disposition (P1 must-fix / non-blocking fix-or-defer) → drive the fixes → re-review on the new head → defer leftovers → merge on explicit ack — run end-to-end by **whichever agent invokes it** (Claude Code or Codex; no cross-agent hand-off, the Codex-vs-Claude choice is the user's at invocation time, purely for context-window management). **Provider-agnostic, skill-driven, zero determinism in the loop** (North Star §1) — the disposition contract is *not* forked: it reuses the single source of truth `planning-vertical-slice/references/git-workflow.md` §"Agent-driven pre-merge gate", the same gate `closing-vertical-slice` §10a and `writing-sprint-retrospective` §8a run at close. **Targeting (fork 2):** manifest-free — resolves the repo from `git rev-parse --show-toplevel` (or `--repo-root`), no `manifest_require`; the only deterministic change is an optional `--repo-root DIR` target added to the PR helpers (`sd_pr_state`/`sd_pr_review_comments`/`sd_pr_merge`/`sd_remote_check`, extracted to the shared `_sd_repo_target` parser the issue helpers retrofit onto), defaulting to `.canonical.root` so slice/sprint callers stay byte-identical. **Deferred by design:** remote `--repo owner/repo` (gh `-R`) targeting. Design of record: `docs/agent-driven-program/specs/SS-9-work-pr.md`. Demand-validated by the hand-maintained Codex PR-fix prompt this collapses ([[feedback_codex_runs_pr_fix_cycle]]).
**Closes:** `#92`.
...
| ~~**#10**~~ | coordinating-parallel-slices | independent (demand-gated) | **SS-6** | ✅ **WONTFIX 2026-07-02** (post-#93 recommend-by-default dogfood, `/council`+`/critique`). Purely demand-gated; sole maintainer attests no current parallel-slice need — analog of #58 wontfix precedent (`593db05`). Trigger recorded on the issue: re-open/re-file when a real multi-worktree parallel-slice job lands. |
| **#37** | grilling domain-language + ADR thresholds | partial (principle) | **SS-6** | **DECIDED 2026-07-02** (post-#93 dogfood): **build legs 2 + 3**, wontfix 1/4/5. Leg 3 = strict 3-part ADR threshold; leg 2 = code-contradicts-claim with **mode-dependent source-of-truth** (dev → code authoritative; vision-planning → doc/vision authoritative). Open as a 2-leg build. |
| **#38** | handoff suggested-skills + redaction | partial | **SS-6** | **DECIDED 2026-07-02** (post-#93 dogfood): **build all 5 legs** (user override of council's cherry-pick-redaction-only — all five are felt pain: suggested-skills, artifact-refs, redaction, next-session-focus, ephemeral mode). Open as a full build. |
...
| ~~**#88**~~ | ai-mentor + architect-critic: agent-driven "You-are-here" orientation preamble for dialogue skills (grill-me/council/critique/brainstorm open with **topic / where-it-sits + strategic-weight / why**) | independent (enhancement; **agent-driven, NO determinism**) | **SS-6** | ✅ SHIPPED 2026-06-25 (PR #90, **ai-mentor v2.1.0** + **architect-critic v0.4.0** + **scaffold-onboard v0.11.0**). Pure-prose orientation block embedded at each skill's opening — grill-me/council before the first question/personas, critiquing-spec at Step 4 (after artifact-resolve, one insertion covers sync + `--async`). Triggers: open-of-session + on-demand re-surface ("where am I?"); derive from issue/PR + memory-bank + handoffs, **ask when thin (never fabricate)**. **Axis-B IN scope** (user-chosen): scaffold-onboard `WORKFLOW.md` template seeds the convention so derived projects inherit oriented dialogue/cognitive work — chosen over a heavier Karpathy-style Phase-10 opt-in gate (lightweight convention, not imposed discipline). Clean 2-round bot review via batch-fix-in-one-pass (both findings docs-nits — markdownlint heading false-parse + test-index registration; no product bug) ([[feedback_bot_review_batch_fix_one_pass]]). [[feedback_orientation_preamble_dialogue_skills]] |
| **#86** | scaffold-onboard: `/amend-spec` skill — incremental, **change-driven** MASTER-SPEC amendment for the post-MVP / vNext lifecycle (classify change → impact analysis vs MASTER-SPEC+SRS/BACKLOG → collision-safe ID mint from ledger → *targeted* spec edit + *diff-aware* governance-doc update, vs today's greenfield whole-bundle re-derive) | independent (enhancement; **larger design item** — brainstorm-first) | **SS-8** (own sub-spec) | ✅ **SHIPPED 2026-06-27** (PR #91, squash `a3c8b9c`, tag `scaffold-onboard-v0.12.0`) — brainstorm reframed to a **front-door MVP**: agent-driven classify → impact analysis → *targeted* MASTER-SPEC edit + `## Revision History` + SSoT `phase_record` fold; **diff-aware doc-merge + stable ID minting deferred** (envelope choice — MASTER-SPEC is ID-free, `/scaffold-docs` re-mints wholesale), propagation routes to `/scaffold-docs` + `/plan-roadmap`. Zero new `lib/*.sh`. Codex review pass fixed 3 real draft bugs (invalid roadmap mode, lock-release-on-failed-acquire, EXEC-SUMMARY H1/H2 shape) + `project_mismatch` safety stop. Design of record `specs/SS-8-amend-spec.md`. Demand-validated by PulseDB v0.5.x→vNext. |
| **#92** | scaffold-dev: expose the #82 pre-merge gate / finding-disposition loop as a standalone, slice-decoupled `/work-pr` command (runnable against an arbitrary PR, not just slice/sprint close) | independent (enhancement; **brainstorm-first**) | **SS-9** (own sub-spec) | ✅ **SHIPPED 2026-06-30** (PR #94, squash `1b0eebb`, **scaffold-dev v0.14.0**, tag `scaffold-dev-v0.14.0`) — `/work-pr <PR>` (`working-pull-request`): full review-fix-merge loop run end-to-end by whichever agent invokes it (Claude **or** Codex; no cross-agent hand-off), **provider-agnostic, manifest-free, zero determinism in the loop**. Reuses the single `git-workflow.md` §7 contract (no forked copy). Only deterministic change: optional `--repo-root DIR` on the PR helpers via the shared `_sd_repo_target` parser (issue helpers retrofitted onto it), defaulting to canonical so slice/sprint callers stay byte-identical. Codex companion ran the bot-review-fix cycle (3 commits — fixed real bugs: full-URL owner/repo resolution in `sd_pr_review_comments`, single-owner deferral routing, dirty-repo + PR-head-OID checkout safety in the skill preflight, `gh repo view` resolution check); I reviewed all 3 on the real HEAD + re-verified (suite 23/0, dual-publish 155/0) before squash-merge ([[feedback_codex_runs_pr_fix_cycle]]). Remote `--repo owner/repo` deferred. Design of record `specs/SS-9-work-pr.md`. |
| ~~**#93**~~ | recommend-by-default decision policy across the 4 decision-surfacing skills (grill-me / council / critiquing-spec / orchestrate gates) — one firm **vision-grounded recommendation** + one-line rationale by default, `accept`/`rebut`/`defer` dispositions, `--neutral` opt-out | independent (cross-cutting interaction policy; **agent-driven**) | — (marketplace convention) | ✅ **SHIPPED 2026-07-01** (PR #95, squash `0692dad`, **ai-mentor v2.2.0** + **architect-critic v0.5.0** + **scaffold-dev v0.15.0**). Single SoT `docs/conventions/recommendation-policy.md` shipped as **byte-identical per-plugin copies** + repo-root parity test `tests/test-recommendation-policy-parity.sh` — repo-root `docs/` does **not** install, so the issue's literal single-referenced-doc was not viable ([[reference_cross_plugin_shared_doc_pattern]]). ai-mentor stays **orthogonal** (grounds via its soft orientation-derivation chain, no manifest dep); architect-critic reframes `dismiss`→`defer` + attaches a recommended disposition per challenge (scorer internals unchanged); scaffold-dev gates carry a MASTER-SPEC-cited recommendation (additive, §15 authority preserved). Codex ran the initial fix cycle (`9ff7128`); I **independently reviewed the real HEAD and found + fixed 9 valid bot findings across 2 rounds** — Codex's "0 unresolved" was inaccurate (positional-append deferred **data-loss**, stale `ARCHITECT_CRITIC_ARGS` leak, final-round phantom "next round", `"you synthesize"` mis-aliased as neutral, `--neutral` not forwarded to grill-me gates, missing best-practice label, backtick command-sub test bug, defer not exposed at gates) before squash-merge ([[feedback_codex_runs_pr_fix_cycle]], [[feedback_bot_review_convergence_judgment]]). |

| **#96** | architect-critic critiquing-spec: Step 8 prescribes nonexistent `arc scorer_score` (real fn `scorer_score_rebuttal`) → rebuttal path fails; + F2 `scorer_decide` vocab divergence + F3 deterministic-scorer/`pp-e72993dfb626c518` tension | independent (bug) | — (post-#93 dogfood) | open — found dogfooding #93 (2026-07-02); fix = SKILL L389 → `scorer_score_rebuttal`. See `docs/agent-driven-program/dogfood-recommendation-policy-2026-07-02.md` |
