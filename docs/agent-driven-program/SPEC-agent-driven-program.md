...
| **#79** | scaffold-dev: explicit count-aware `auto:` form (`expected: ran ≥N`) for runners outside the zero-collection allowlist | independent (enhancement) | **SS-6** | open (the #74 escape hatch; cross-plugin minor — scaffold-onboard grammar `sf_demo_parse_line` + scaffold-dev `sd_verify_auto_step` at all 3 gate sites) |
