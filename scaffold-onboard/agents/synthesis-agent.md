...
description: Synthesize one scaffold-onboard artifact from the source material named in the invocation prompt per a synthesis brief. Supports first-author MASTER-SPEC synthesis from the onboarding digest, plus post-MASTER-SPEC artifacts (governance doc, roadmap-slice, memory-bank file, CLAUDE.md, or EXECUTIVE-SUMMARY.md). Reads the named sources + the brief, writes the artifact to the given absolute path honoring the required-section contract, mints/cites IDs from the provided ledger slice when applicable, never emits fill-in markers, and returns a compact ID-ledger JSON. NEVER runs git and NEVER invokes Task (no subagent nesting).
...
## Binding rules
- If the prompt is for `MASTER-SPEC.md` in first-author mode, no existing spec exists. Use the embedded MASTER-SPEC brief + onboarding discussion digest in the prompt as the source of truth and write the named output path. (MASTER-SPEC is always authored whole — partial reconcile was decommissioned, #58 wontfix.)
