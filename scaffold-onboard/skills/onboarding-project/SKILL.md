...
- **Resume protocol.** On entry, call `sf state_mode`. If `resume`:
  ...
  - **Missing-record repair pass (all resume variants):** inspect phases up to AND INCLUDING the current resume phase (or all 10 phases on a close_pending resume). For each phase P: if `sf state_read_phase_record <phase_id>` returns `null` but the phase has any stored answers (any key in `.answers` with that phase's numeric prefix is non-null), re-author that phase record from the stored answers in conversation and persist it with `sf state_write_phase_record` before advancing. This explicitly covers the current phase: if a crash occurred after the current phase's last answer was persisted but before `sf state_write_phase_record` + `sf state_advance_phase` ran, the state remains `in_progress`/`current_phase=N` with all answers present but no record and no unanswered required question — the repair pass detects this (record missing + no remaining required questions), re-authors the record, then advances so the resume continues cleanly at phase N+1. No gate evaluation needed — this sidesteps the gated-Phase-6 false-incomplete problem entirely (the helper is now non-gating, so a gated-out branch won't be incorrectly required).
