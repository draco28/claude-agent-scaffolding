...
**Agent-unavailable model:** dispatch the per-slice sub-agent → if no Task tool (headless), the conducting agent drafts the slice **inline in the main context** from the same brief → on a structurally invalid draft, re-draft once → then **fall back to the standard R1.C interactive authoring (§4.3, §16.2): the user authors the slice's Scope + demo criteria directly.** There is no deterministic slice *renderer* (the roadmap JSON→markdown *formatter* in `lib/roadmap.sh` is mechanical and unaffected — it formats the agent-authored slice data). Roadmap is the one surface where the fallback is **interactive (human-authored) authoring, not hard-fail** — interactive authoring is human-driven, not a deterministic content renderer, so it remains a legitimate fallback under SS-7 (unlike the memory-bank / governance / EXEC-SUMMARY surfaces, which hard-fail because their only non-agent path *was* a deterministic renderer).
