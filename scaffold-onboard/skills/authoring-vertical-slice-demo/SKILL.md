...
Pattern mode also covers **numeric predicates the slice-close orchestrator judges** (not greps): `count > 0`, `> 5 rows`, and `ran ≥N` — assert that at least N tests actually ran. Reach for `ran ≥N` as the portable count guard for runners scaffold-dev's `exit 0` zero-test check can't see (rspec, mocha, `mvn test`, wrapper scripts); ASCII `ran >=N` is equivalent and N defaults to 1. See `references/auto-grammar.md` §2.2 / §3.10.
...
- **scaffold-dev's `closing-vertical-slice`** is your downstream consumer at slice-close time — parses demo lines directly, executes the `auto:` lines, surfaces the `user:` lines for human walkthrough. Not invoked from this skill. Note the two-level distinction: work-item ACs are verified mechanically/deterministically by `implementation-checking`; slice demo criteria are agent-judged acceptance checks by `closing-vertical-slice`.
