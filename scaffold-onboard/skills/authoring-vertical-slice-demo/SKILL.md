...
description: Author `auto:`/`user:` demo criteria for a named vertical slice per the v0.2 R3 grammar (SPEC §5.6 + §9). Use this when the user wants to author demo criteria for slice X, asks "what should this slice demo?", wants to set up demo verification for VS-N.M, or asks to add a demo step to a vertical slice. Validates each line via `sf_demo_parse_line`, idempotently appends to the named slice (text-equality match, no duplicates), and writes to one of two target shapes: the `vertical_slices[].demo_criteria[]` array in `project-roadmap.json` (state mode, used by `planning-project-roadmap` during R1.C before ROADMAP.md exists) OR the `##### Demo criteria` subsection of the slice's `#### VS-<id>:` block in ROADMAP.md (markdown mode, used by scaffold-dev's orchestrator at slice planning and by direct post-emission top-up). Auto-detects the right mode from the state-file checkpoint; user can override via `--target=state|markdown`. Grammar uses the literal U+2192 arrow character (→), NOT the ASCII `->` digraph. Never invokes architect-critic; no dedicated slash command.
---
