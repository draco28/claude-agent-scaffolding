...
The 8 derived memory-bank files + CLAUDE.md are authored by **sub-agent synthesis — the only derivation path** (the deterministic `--fast` renderer was removed in v0.8.0, SS-7). This section describes the orchestration logic you (the orchestrator session reading this skill) must execute. Bash cannot dispatch sub-agents; this logic lives here as prose instructions.
...
Dispatch the 8 artifacts in Wave 4. All artifacts are independent of each other (no sequential ID-dependency within this skill), so they can be dispatched in parallel. Maintain a running ledger from the start:
...
if [[ "${regenerate:-0}" == "1" ]]; then
  ...
  pushd "$(sf_resolve_output_path memory_bank .)" >/dev/null || sf_log_error "could not enter the memory_bank root for harvest migration"
...
**Wave 4 — all 8 artifacts in parallel** (all model `sonnet`; `routes_to` per brief):
...
```bash
sf_claude_md_generate       # renders CLAUDE.md.tmpl: composition gates + conditional
                            # Karpathy section per phase_10.4.include_karpathy (§6).
                            # This is the ONLY path that honors the Karpathy opt-in —
                            # it must run on every /scaffold-project, fast or not.
