...
### Changed
- **#104 — consistent MASTER-SPEC reasoning structure.** The Phase-10 synthesis brief now preserves concrete critic/grill traces only when they changed a decision, emits phase-local rejected alternatives, consolidates genuinely cross-phase architectural constraints, requires recorded computational/propagation/type semantics for Phase-3 entities, and always produces a source-grounded post-MVP horizon appendix. Parser anchors and the public phase API remain unchanged; Phase-10 extraction now treats that mandatory top-level appendix as an out-of-phase boundary. Explicit anti-invention rules prevent synthetic audit IDs, alternatives, formulas, and future work. Contract assertions plus a populated Phase-10 behavioral eval pin the new quality bar.
...
- **CLAUDE.md is now mechanically generated, not synthesized** (`scaffolding-memory-bank` §13). With `--fast` removed, the only path that honored the Phase-10 Karpathy opt-in (`phase_10.4.include_karpathy`) + plugin-composition gates was gone — the synthesis brief (`CLAUDE.brief.md`) never handled them. CLAUDE.md is a structured/conditional router file with a verbatim-attribution block, so it is emitted by the kept `sf_claude_md_generate` in the §13 finalize alongside `.claude/settings.json` and the AGENTS.md managed section (the synthesized memory-bank set drops from 9 artifacts to 8; the now-orphaned `CLAUDE.brief.md` is removed). (PR #62 review)

### Fixed
- **`/scaffold-project --regenerate` data loss on legacy upgrade:** the §13 harvest migration ran in a forking `( cd … )` subshell, so the global `_SF_MB_MIGRATED_TO_KNOWN_ISSUES` flag it sets never reached the separate `--force` live/static seed subshell — the just-migrated `09-known-issues.md` was then force-overwritten, dropping the migrated harvested notes. The migration now runs non-forking via `pushd/popd` so the flag persists into the seed step. Guarded by a new dispatch test. (PR #62 review)
- **Karpathy opt-in could be silently dropped from CLAUDE.md** under synthesis-only generation — fixed by the mechanical-generation change above (see Changed). (PR #62 review)
- **Headless inline EXEC-SUMMARY fallback** (`onboarding-project` §8) now authors from the **full MASTER-SPEC following the EXEC-SUMMARY brief**, not from MASTER-SPEC's pinned `## Executive Summary` placeholder (a one-sentence stub at close) — preventing a thin placeholder from being checksum-pinned. The synthesized write-back rejection now **stops the close** (re-dispatch-once → hard-fail) instead of warning and falling through to a `complete` close with an unrefreshed summary. (PR #62 review)
- **Roadmap §16 failure model reconciled** (`planning-project-roadmap`): the intro now matches §16.2 — a structurally invalid slice draft re-drafts once, then falls back to the standard R1.C **interactive (human-authored)** authoring rather than hard-failing. Interactive authoring is human-driven (not a deterministic content renderer), so it stays a legitimate fallback under SS-7; only surfaces whose sole non-agent path was a deterministic renderer hard-fail. (PR #62 review)
- **Test-fixture + guard hardening:** `seed_governance_docs_fixture` now routes each doc through `sf_resolve_output_path` with the real §11 logical names (so dual-repo routing regressions aren't hidden); the command-wrapper `--fast` parse guard widened to catch `FAST=0` / `local FAST=…`, not just `FAST=$`. (PR #62 CodeRabbit)
- **CLAUDE.md mechanical generation hardened** (`scaffolding-memory-bank` §13, follow-on review rounds): the §13 opening no longer mislabels CLAUDE.md as synthesized; the write is routed via `( cd "$(sf_resolve_output_path claude_md .)" && sf_claude_md_generate )` so a dual-repo run lands it at the routed root, not pwd; and `lib/compose.sh` is now sourced in §13 setup so `_composition_args` → `sf_compose_detect_architect_critic` is defined (otherwise the `/critique` block was dropped from CLAUDE.md for architect-critic projects). (PR #62 review)
- **Harvest-migration pushd hard-stops on failure** — `sf_log_error` only logs, so a failed routed `pushd` previously fell through and ran the migration in the caller's cwd; now `if ! pushd …; then sf_log_error; return 1; fi`. (PR #62 review)
- **EXEC-SUMMARY consumer is best-effort, not hard-fail** (`/scaffold-project`, `/scaffold-docs`): aligned with SS-7 spec §4 — the consumer produce-once-if-missing has no inline fallback when headless and does **not** hard-fail (unlike the onboarding-close producer, §8). EXEC-SUMMARY is optional enriching context there; MASTER-SPEC is the SSoT, so on a failed/absent produce it warns and derives from MASTER-SPEC only. The consumer-vs-producer asymmetry is now explicit in both skills. (PR #62 review)
- **Roadmap §16.2 corrective retry made executable** — the per-slice failure handler now performs the re-dispatch-once the §16 intro promises before falling back to interactive authoring. (PR #62 review)
