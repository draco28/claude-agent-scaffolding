...
- Skill restates current position ("Resuming at Phase 4, question 3 of 5") or equivalent.
...
- After confirmation in transcript (simulated by the judge: the target subagent should pause for user confirmation; the eval harness pre-injects `yes` as the user's second turn).
...
- **No composition.json entry for architect-critic** (v0.2 dropped it per ac settlement #1).
- Mock the plugin cache filesystem: create `${HOME}/.claude/plugins/cache/test-marketplace/architect-critic/0.2.0/skills/critiquing-spec/SKILL.md` (stub content) so the filesystem probe per SPEC §12.2 returns `v0.2`.
...
- No `composition.json` read or write attempt for the architect-critic entry (judge inspects the file-access pattern in the transcript: composition.json may be read for ai-mentor/superpowers detection per §12.2, but the architect-critic decision must come from the filesystem probe).
...
- Multi-session interrupts (covered separately in `evals/scaffolding-memory-bank.md` resume tests, if applicable)
