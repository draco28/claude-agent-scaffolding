...
no-flag case (synthesis derivation that preserves live state files). Memory-bank
derivation is agent-synthesis only — there is no `--fast` deterministic path (v0.8.0).
