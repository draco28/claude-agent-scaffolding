...
test_commands_have_no_fast_flag() {
  ...
  # Match any FAST assignment/parse (FAST=$1, FAST=0, local FAST=…), not just the
  # literal FAST=$ — a narrow guard would miss real parser reintroductions. FAST must
  # be at line-start or after whitespace (optionally `local `), so SF_SYNTH_FAST= does
  # not false-match.
  if grep -qE '(^|[[:space:]])(local[[:space:]]+)?FAST=' "$project" "$docs"; then echo "  ✗ a command still parses a FAST flag"; ok=0; fi
...
test_memory_bank_synthesis_regenerate_migrates_harvest_before_dispatch() {
  ...
  wave_line="$(printf '%s\n' "$body" | awk '/Wave 4 — all 8 artifacts/{print NR; exit}')"
...
# PR #62 Codex P2 (data-loss) — _sf_mb_migrate_harvested sets the GLOBAL flag
# _SF_MB_MIGRATED_TO_KNOWN_ISSUES, which sf_memory_bank_seed_live_static --force
# reads to avoid clobbering the just-migrated 09-known-issues.md. The migration
# must therefore run NON-FORKING (pushd/popd) so the flag survives into the seed
# step; a `( cd … && _sf_mb_migrate_harvested )` forking subshell would discard the
# flag and --force would overwrite the migrated notes. Guard against a regression to
# the forking form.
test_harvest_migration_runs_non_forking_so_flag_survives() {
  echo "test_harvest_migration_runs_non_forking_so_flag_survives:"
  local body ok=1
  body="$(_extract_section_bash "$MB_SKILL" "## 13")"
  # Must NOT wrap the migration in a forking subshell of the form
  # `( cd "$(sf_resolve_output_path memory_bank …)" && _sf_mb_migrate_harvested )`.
  # Match the actual code form (cd + the resolve call + && + the migration on one
  # line), not prose mentioning "( cd … )".
  if printf '%s\n' "$body" | grep -qE 'cd .*sf_resolve_output_path memory_bank.*&&.*_sf_mb_migrate_harvested'; then
    echo "  ✗ harvest migration is wrapped in a forking ( cd … ) subshell — the migrated-09 flag would be lost"; ok=0
  fi
  # Must run it via pushd (non-forking) so the global flag persists into the seed step.
  if ! printf '%s\n' "$body" | awk '
      { n++ }
      /pushd .*sf_resolve_output_path memory_bank/ { route=n }
      /_sf_mb_migrate_harvested/ { if (route && n - route <= 4) ok=1 }
      END { exit (ok?0:1) }'; then
    echo "  ✗ harvest migration is not run via a non-forking pushd memory_bank block"; ok=0
  fi
  if [[ "$ok" == "1" ]]; then PASS=$((PASS+1)); echo "  ✓ harvest migration runs non-forking — _SF_MB_MIGRATED_TO_KNOWN_ISSUES survives into the --force seed"; else FAIL=$((FAIL+1)); fi
}

# PR #62 Codex P2 (correctness) — CLAUDE.md must be MECHANICALLY generated by
# sf_claude_md_generate (the only path that honors phase_10.4.include_karpathy +
# composition gates), NOT dispatched as a synthesis artifact. Assert §13 calls the
# helper and does NOT list CLAUDE as a synthesized wave artifact.
test_claude_md_mechanically_generated_not_synthesized() {
  echo "test_claude_md_mechanically_generated_not_synthesized:"
  local body ok=1
  body="$(_extract_section_bash "$MB_SKILL" "## 13")"
  if ! printf '%s\n' "$body" | grep -qE 'sf_claude_md_generate'; then
    echo "  ✗ §13 finalize does not call sf_claude_md_generate (Karpathy opt-in would be lost)"; ok=0
  fi
  # The synthesized-artifact table must not route a CLAUDE artifact to claude_md.
