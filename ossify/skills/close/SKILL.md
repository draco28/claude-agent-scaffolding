...
2. **`oss doctor` must be green on `schema` and `replay`.** A close *mutates*
   ...
   **`oss doctor` no longer reports the advisories.** A held lock, a pending
   ...
   outstanding fakes at release step 3. **Read the lock yourself, before any
   scope's first step:** if `"$(oss state_path).lock"` exists as a directory,
   ...
   kills the ceremony at rc 3. Do not
