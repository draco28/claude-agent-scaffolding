...
resolution makes this role unavailable. Never use a cache-directory name.
...
`ossify/.claude-plugin/plugin.json`, use that checkout's `ossify/` directory and
manifest. Otherwise, on Claude Code only, read
...
partial result. Version equality is the boundary: do not audit same-version
content.
...
skills. For prior verbs, report that they ran through the mismatched binary;
do not recursively diff `bin/` or `lib/` and do not infer implementation
impact. If history was compacted or the expected root is unavailable, say the
impact view is partial.
