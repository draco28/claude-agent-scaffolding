...
|---|---|---|
| 1 | **System shape & deployment topology** | What runs where? One process, client+server, desktop app, CLI, job + worker? What is the deployment unit? **Splitting work that could run in one process into a second deployable needs the measured pressure that requires it** (`plan-spine/references/codebase-design.md` §3) — "it will need to scale" is not an answer. A topology the product *imposes* — a mobile or desktop client against a hosted backend, a browser front end — is not a split: the constraint forces **one** deployable per execution context, and every further deployable inside a context is a split that needs the evidence. |
