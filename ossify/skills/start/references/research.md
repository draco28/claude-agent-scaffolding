...
research **verified** go in the ADR's `### Verified claims` (citing the
research file, not re-quoting the source); claims still **unverified** go in
`### Unverified claims` with a revisit trigger. Silence is the defect, not the
