...
<!-- Executed against tracked files by the release-close boundary audit
     (close/references/boundary-audit.md) at every release close of an
     observed-public repo — the agent reads each rule and matches it against
     git ls-files. Authored at onboarding so the file exists from day one. -->
...
**executes** (`close/references/boundary-audit.md`) — the agent reads each
rule and matches it against the tracked file list, from any checkout, with no
private context needed. Author it at onboarding so the first release close has
something to read.
