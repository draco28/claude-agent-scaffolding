...
| `adopt-multi-repo` | `adopt` | A3's per-repo-plus-workspace cleanliness sweep (dirty tree and off-default-branch checks run against every declared repo, never canonical alone) and the two multi-repo conversions that follow the gates: a baseline recorded as one SHA per declared repo rather than one SHA for the whole product, and C3 back-deriving bones from every declared repo's `docs/adr/` directory, aggregated |
