...
| `doctor-provenance` | `doctor` | the provenance surface (`provenance-check.md`): the answering `oss` binary and the loaded skill body reported as two separate identities, each with its resolved path, even when they agree; the comparison reference selected from context (the checkout's own manifest when working inside one, the installed record in a consumer project) and named, with `skip:` and a `partial` roll-up whenever a reference fails to resolve either half — an identity or a readable comparison root; the comparison run unconditionally over the union of skill bodies under both roots, one line per body including one-sided ones, saying so plainly when nothing differs, with the rerun decision left to the operator and never a blanket redo or a ceremony ruled safe; and the surface's own limits stated — it is opt-in, nothing re-resolves the plugin table mid-session, and it cannot report its own absence — so a clean line describes the moment doctor ran |
| `wayfinder-judgment` | `wayfinder` | routing a loose question into a map or refusing one; fog that stays fog versus a ticket; how an existing map's frontier is worked and which tickets a subagent may touch; and the ladder deciding which tracker a map lives on |
