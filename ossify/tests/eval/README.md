...
| `boundary-audit-integrity` | `close` | the release-close boundary audit (`boundary-audit.md`): observed-visibility gate held against the manifest's word; no silent narrowing (missing artifact / missing tool = finding, not skip); scan-first untracked classification; never-auto disposition with the inventory-recorded override; and calling a clean close clean |
