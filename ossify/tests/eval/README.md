...
| `doctor-provenance` | `doctor` | the provenance surface (`provenance-check.md`): the answering `oss` binary and the loaded skill body reported as two separate identities, each with its resolved path, even when they agree; the comparison reference selected from context (the checkout's own manifest when working inside one, the installed record in a consumer project) and named, with `skip:` when neither resolves; `SKILL.md` deltas reported one ceremony at a time with the rerun decision left to the operator, never a blanket redo or a ceremony ruled safe; and the surface's own limits stated — it is opt-in, and nothing re-resolves the plugin table mid-session, so a clean line describes the moment doctor ran |
