...
expected_reason: the already-exists guard (issue 133) must be checked against EVERY hosting repo, not canonical alone — svc-billing still carries a stale branch named spine/r1.s4-invoice-export-pipeline from a botched earlier attempt, even though canonical's copy was deleted and it would cut cleanly on its own; a ceremony still doing the old canonical-only lane never inspects svc-billing, so it would treat this as a fresh spine, cut a new branch in canonical, and leave svc-billing's stale one undetected underneath — a wrong answer distinguishable from the correct one regardless of iteration order, since canonical alone gives no evidence of the collision
---
