...
expected_findings: the README section is a semantic finding (describes a moat item without containing it); the override is recorded in the PRIVATE boundary inventory with the reason — not in project-state.json, not by editing the README finding away — and re-surfaces as a standing warning at the next close
---
