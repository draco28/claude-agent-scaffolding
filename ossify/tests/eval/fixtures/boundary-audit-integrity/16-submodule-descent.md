...
- **`vendor/ui-kit`**, pinned at `77b0a3c`. The working copy of the canonical
  was made with a default `git clone`, so `vendor/ui-kit/` is an empty
  directory: nothing of that submodule's pinned tree is on disk.
