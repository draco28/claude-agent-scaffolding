...
oss_state_mutate() { # $1=state-file $2=op $3=payload-json [$4=mint-spec]
  ...
  if ! mkdir "$lock" 2>/dev/null; then
    echo "oss: state locked ($lock exists) - another ceremony is mutating. If none is, the lock leaked from an interrupted run: the /doctor skill reports it, with staleness, once it is >30min old; or rmdir it yourself after confirming no ceremony is running" >&2
    return 3
  fi
