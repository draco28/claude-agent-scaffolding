...
4. **Semantic hook configuration review.** If either project Claude settings file exists, read only decoded values under its `hooks` entries. Interpret JSON escapes in each hook command before judging it; do not execute a command or handler. When a decoded hook command directly names a project-local handler, read that file as text too. A malformed settings file, unreadable handler, or handler outside the project is an incomplete/uninspected review, never a clean result. Redact reported excerpts under the audit's normal secret rules, and do not treat rule-shaped values outside `hooks` as hook behavior.
5. Run the deterministic rule engine: `csa rule_engine_scan_all "$PWD"` iterates concrete targets × applicable rules; it does not parse Claude settings hook configuration.
6. Compute durable `finding_uid` (no line number) + per-run `dedup_fingerprint`: `csa finding_uid <...>` / `csa dedup_fingerprint <...>` (per scanner finding).
7. Tag scanner findings NEW vs PERSISTED via `csa baseline_tag <findings>` against state.json's `findings` registry.
8. Filter suppressed scanner findings via `csa suppress_filter <findings>` (unless `--show-suppressed`).
9. Write the scanner report file `.claude/audits/<date>-<NN>.md` and return chat summary via `csa report_render_markdown` + `csa report_render_chat`. Add the semantic hook configuration review as a clearly labelled prose section; its observations are actor-owned and do not receive scanner JSONL IDs, baseline tags, or suppressions.
10. Update `state.json` (last-audit-date, scanner findings registry with GC per references/severity-rubric.md §GC, `self_integrity.state_mtime_at_last_audit` per references/auto-fix-policy.md §tamper).
11. If any rules failed to load, emit prominent chat banner: `"⚠ N rule(s) failed to load; results incomplete. See report for SCANNER-001 findings."`
12. Emit chat summary inline (per references/severity-rubric.md §chat-summary-format). Never call the overall audit clean when the semantic hook configuration review contains a finding or is incomplete/uninspected.
