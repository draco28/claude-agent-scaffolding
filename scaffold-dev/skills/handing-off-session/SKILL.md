...
description: Compose a forward or return session handoff doc capturing ephemeral pre-codification state for out-of-slice transitions per SPEC §6b — sprint-boundary carry-forward, slice-boundary breath, mid-slice context-bloat recovery, or mid-slice bug-fix / tech-debt detour. Writes to `<ai-workspace>/.workspace/handoffs/<scope>-<purpose>-<short-id>.md` per §6b.1 naming convention (4-char hex short-id, no timestamp, `-return.md` suffix for return handoffs); renders all 10 standardized sections from §6b.5 with verbatim section titles; lazily `mkdir -p`'s the `handoffs/` subdir on first invocation (workspace-init owns the parent `.workspace/` namespace and seeded the `.gitignore` line `.workspace/handoffs/` per its §8.3); detects forward-vs-return from `$ARGUMENTS` (`--return-of <name>` or `--return <short-id>`) plus conversation context; refuses to write with an empty section 4 (the value-add over memory bank per §6b.5); reads `.gitignore` on exit and warns the user if the `.workspace/handoffs/` pattern is missing (never auto-edits — that's workspace-init's lane). Use this when the user says `handoff to next session`, `hand this off`, `context bloated`, `fresh session for VS-N.M`, `compose a handoff`, `write a return handoff`, or invokes `/handoff [--scope ...] [--purpose ...] [--return ...]`. NEVER edits `.gitignore`, NEVER commits anything (the storage path is gitignored by design), NEVER invokes itself recursively, and per SPEC §6b.7 the `scaffold-dev:implementer-agent` subagent is explicitly forbidden from invoking this skill.
---
...
- No workspace-init pairing manifest exists. The handoff storage path is anchored at `<ai-workspace>/.workspace/handoffs/`; without a manifest there is no anchor. Refuse with the same verbatim string `planning-vertical-slice` uses (see §3.1).
...
- Read the existing forward handoff before composing the return — confirm the short-id, read the forward's Section 8 (Next intended action(s)) to ground the return's Section 2 (Purpose: "what the fork session accomplished against the forward's next-action").
...
### 5.2 Compose the 10 sections (verbatim titles per §6b.5)

Render `templates/handoff.md.tmpl` via `lib/render.sh`'s `{{var}}` substitution. The 10 sections MUST appear with these EXACT titles, in this order, as `##` level markdown headings:
...
2. **Purpose** — one paragraph: why handing off. Forward handoffs name the reason for the out-of-slice transition (sprint boundary / context bloat / bug-fix detour). Return handoffs name what the fork session accomplished against the forward's Section 8.
...
8. **Next intended action(s)** — REQUIRED, user-prompted (§7 below). Single specific action OR ranked list of options. Concrete, actionable, names the cursor unambiguously.
9. **Anti-actions** — explicit "do NOT do X" warnings. Carries over §6b.2 / §6.6 anti-patterns AND any session-specific anti-actions the user names (e.g., "do NOT retry the auth fix attempt that failed for reason Y").
10. **Return-handoff template stub** — REQUIRED on forward handoffs; populated with sub-headings `Summary`, `Deferrals`, `Cautions`, `Memory bank promotion candidates` (per the master-session `docs/HANDOFF-scaffold-dev-build.md` prototype, which IS the working example of this pattern). On return handoffs, this heading still appears (parser-friendly 10-section contract is binding) but rendered as `n/a — this IS a return handoff` or equivalent.

**10-section invariant (binding per eval cross-scenario):** EVERY handoff file you write — forward OR return — contains all 10 headings in order, with the exact titles above. The judge scans for each title as a level-agnostic markdown heading (`##` or `###`); a missing or renamed section is a FAIL. Section 8's `(s)` suffix is the only paraphrase the judge accepts ("Next intended actions" with or without parens).
...
{{return_stub}}         section 10 body (forward: stub template; return: n/a note)
```
...
## 7. Section 8 — REQUIRED, user-prompted

Section 8 ("Next intended action(s)") names what the next session should do first. Per the §6b.5 description, it's a "single specific action OR ranked list of options" — concrete enough that the receiving session can act without re-deriving.

### 7.1 Auto-extract from state cursor

If the state cursor names an `ACTIVE_WORK_ITEM`, the default Section 8 candidate is the orchestrator action that was about to fire when the handoff was triggered:
...
Present the auto-extracted candidate and ask: *"Section 8 draft — is this the next intended action? Edit if not."*.
...
## 8. Write the file (atomic mv pattern)
...
## 9. Gitignore exit-check (binding, read-only)
...
- **Dispatching subagents via the `Task` tool.** The skill is a one-shot composer, not an orchestrator. No subagent dispatch belongs here. If a sub-task surfaces (e.g., user wants the next session pre-briefed via a subagent), surface that as a Section 8 candidate, not via Task dispatch from inside this body.
...
When in doubt, prefer composing-and-surfacing over silent invention. A handoff is a deliberate ceremony — the user-prompts in §6 and §7 are the correct interaction shape, not friction to optimize away. The 10-section invariant + the filename pattern + the gitignore read-only contract + the section-4 non-empty discipline together make the handoff parser-friendly, durable, and aligned with the §6b.4 chain model; cutting any of them corrupts the value the escape valve was designed to deliver.
