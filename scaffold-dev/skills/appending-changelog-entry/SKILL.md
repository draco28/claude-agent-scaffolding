...
description: Append a Keep-a-Changelog 1.1.0 bullet entry to `CHANGELOG.md` under the `## [Unreleased]` section, in one of the six standard categories (Added / Changed / Deprecated / Removed / Fixed / Security), per SPEC §7.1. Manifest-routed: changelog is production-facing, resolves via `routing.changelog` to `<canonical>/CHANGELOG.md`. Reads the file before mutation to locate `## [Unreleased]` and its category subsections; prompts the user for category AND entry text (never silently picks either); appends a single `- <entry text>` bullet under the chosen category heading inside `[Unreleased]`, leaving released sections (`[X.Y.Z] - YYYY-MM-DD`), file header, and link references at the bottom structurally unchanged. Refuses fail-fast when `CHANGELOG.md` is absent (does NOT auto-create — scaffold-onboard's `/scaffold-docs` seeds the Keep-a-Changelog template per §16.2) and surfaces a remediation hint naming both the missing path AND the literal `/scaffold-docs` slash-command token. Use this when the user says `add changelog entry`, `append to changelog`, `log changelog`, `changelog: <entry>`, or invokes a future `/changelog` slash command. Does NOT cut a release section, does NOT auto-create link references, does NOT mutate released versions — those are deferred to v0.2 or handled manually.
---
