...
description: Author a new SRE-style operational runbook under `<canonical>/docs/runbooks/<topic-kebab>.md` per SPEC §7.1 — six required sections (Overview, Symptoms, Immediate response, Diagnosis, Mitigation, Postmortem link). Manifest-routed via `routing.runbooks` to canonical (runbooks are production-facing operational docs). Extracts the topic from the trigger phrase when supplied inline (`author runbook for redis-cache-stale-after-failover`) or prompts the user to disambiguate when the topic is absent (`write a runbook`). Checks for filename collision BEFORE writing; on collision, refuses to overwrite and surfaces a menu with at least two options — (1) append a new dated section to the existing runbook OR (2) author under a different filename — waiting for user pick rather than auto-deciding. Renders `templates/runbook.md.tmpl` (Phase 2 T2.5) with section-by-section user-supplied content; each section must have substantive content (not "TBD"). Use this when the user says `author runbook for X`, `write a runbook`, `create operational runbook`, `write runbook`, or invokes a future `/runbook [topic]` slash command. Does NOT execute runbooks (operator behavior), does NOT auto-extract content from postmortems, does NOT cross-link runbooks — all deferred to v0.2.
---
