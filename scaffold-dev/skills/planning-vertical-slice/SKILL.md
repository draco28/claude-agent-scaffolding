...
- **grill-me (gate 2, SPEC §16.4 offer 2):** *"Specs authored (N items). Want to grill-me on the specs before adversarial review? (yes/no, default no)"* — carry a §3.4 recommendation (*"Recommended: <yes|no> — <one-line, cited>"*; a recommended *no* may auto-resolve per §3.4 triage with digest line) — on **yes** → `Skill(ai-mentor:grill-me)` with `target=specs` (pass `--neutral` when `neutral_mode=true`, and `--walk` when `walk_mode=true`).
...
2. End the turn and wait (skip the wait when §3.4 triage auto-advanced this gate with its digest line — predicate-clean audit gates proceed directly). If the user types `skip` (case-insensitive): log the skip in the slice README and proceed to §8.
3. Otherwise, set a **fresh** env bridge (never inherit — a stale `--spec`/`--close`/`--async` from a prior call would leak into this sync author-depth audit): build the flag string from both mode conditions — `args=""; [[ "${neutral_mode:-false}" == true ]] && args+=" --neutral"; [[ "${walk_mode:-false}" == true ]] && args+=" --walk"`; if either mode is true, `export ARCHITECT_CRITIC_ARGS="${args# }"`, else `unset ARCHITECT_CRITIC_ARGS`. Then invoke `Skill(architect-critic:critiquing-spec)` with:
   - `target=spec`
...
2. End the turn and wait (skip the wait when §3.4 triage auto-advanced this gate with its digest line — predicate-clean audit gates proceed directly). On `skip` (case-insensitive): log the skip in the slice README and proceed to §8.
...
Per §4.1, probe ai-mentor first; offer only when detected. When `neutral_mode=true`, drop the `Recommended:` line from the prompt and pass `--neutral` to the grill-me invocation; pass `--walk` too when `walk_mode=true`.
...
- **Silently resolving a grill-me offer** — the three gates (§4.1 / §6.3 / §8.5) auto-resolve per §3.4 triage only WITH a visible `⚡` digest line; under `--walk` they are explicit user-decidable offers. Eval S1's target — silent skip or silent invocation — remains forbidden.
