...
description: Author a new MADR-lite ADR file (status, context, decision, consequences) under the manifest-routed target directory per SPEC §7.1 — product ADRs (architectural decisions about the user's project) route to `<canonical>/docs/adr/` via `routing.product_adrs`; process ADRs (decisions about the agent-workflow itself) route to `<ai-workspace>/docs/adr/` via `routing.process_adrs`. Prompts the user to disambiguate product-vs-process at invocation (never auto-picks); resolves the target dir via `lib/manifest.sh`; scans the resolved dir for existing `adr-NNNN-*.md` files and picks `max(existing) + 1` as the next ADR number (the two series are independent — process-ADR numbering does NOT cross-count product ADRs next door); prompts for a kebab-case title; renders `templates/adr.md.tmpl` with the user-provided context/decision/consequences content; writes the file at `<resolved-dir>/adr-NNNN-<title-kebab>.md`. Refuses fail-fast when the resolved-but-missing ADR directory does not exist (does NOT auto-create — scaffold-onboard's `/scaffold-docs` owns the seed per §16.2) and surfaces a remediation hint naming both the missing path AND the literal `/scaffold-docs` slash-command token. Use this when the user says `record ADR`, `log this decision`, `add architecture decision`, `ADR for X`, or invokes a future `/adr` slash command.
---
