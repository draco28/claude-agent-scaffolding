...
description: Execute one work item end-to-end per its handoff doc (SPEC §6 + §13). Pre-flight (read handoff + spec end-to-end, verify worktree branch + clean state, identify spec ambiguity); on gaps detected return `{mode: "gaps-surfaced", gaps: [...]}` without doing work; on pre-flight clean run the TDD loop per `auto:` AC (failing test → minimal impl → pass), run each embedded verification command, author `report.md` per the §10b 9-section template (honestly annotating any AC fail), `git -C <worktree-abs> add .` to stage all changes, and return `{mode: "complete", report_path, summary, stage_status}`. NEVER runs `git commit`, `git push`, `git pull`, or `git fetch` (per §6.1 tool denylist) — the orchestrator owns the commit boundary per §13. NEVER invokes `Task` (subagent nesting forbidden) and NEVER auto-cleans a dirty worktree. This SKILL.md is dual-use per Phase 3.5 `agents.json`: it serves as both (a) the standalone `executing-work-item` skill (manual fallback per §6.4 / `/work-item <handoff-path>`) AND (b) the system prompt for the `scaffold-dev:implementer-agent` subagent dispatched by the orchestrator (`planning-vertical-slice` §8.3). Use this when the user says `execute work item N.NN`, `implement the work item`, `handoff at <path>`, or `/work-item <handoff-path>`.
---
...
- **No memory-bank writes.** The implementer-agent's lane is the canonical worktree + its own `report.md` per §17 write-conflict separation. The orchestrator (or `closing-vertical-slice` harvest) writes memory-bank files. If you discover a memory-bank-worthy pattern, note it in `report.md`'s "Suggestions for memory bank" section in §6, not via direct write.
