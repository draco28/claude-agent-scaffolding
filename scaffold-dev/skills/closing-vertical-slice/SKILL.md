...
description: Run scaffold-dev v0.1's three-layer slice-close ceremony (SPEC §14) — parse `auto:`/`user:` demo criteria from the VS README, execute each `auto:` line in canonical and record pass/fail (halt-on-first-fail), surface each `user:` step for manual execution and capture the user's pass/fail/partial outcome, invoke architect-critic's `critiquing-spec` skill in-conversation at close depth (per §16.3 moment 2; warn-and-proceed if absent), author `retrospective.md` per §16b's 7-section format, run the §15.2 8-step memory-bank harvest sweeping both work-item `report.md` files AND slice-scoped handoffs at `<ai-workspace>/.workspace/handoffs/vs-N.M-*.md` with source-tag prefixes `[report]` and `[handoff]`, apply approved items with the provenance trailer, and ONLY THEN remove worktrees + delete branches per §11 M2. Use this when the user says `close VS-N.M`, `slice close`, `wrap up the slice`, or `run slice-close ceremony`. Also owns sprint-close cleanup of non-carry-forward handoffs when invoked on the final slice of a sprint (v0.1 cleanup ownership; no separate `closing-sprint` skill).
---
