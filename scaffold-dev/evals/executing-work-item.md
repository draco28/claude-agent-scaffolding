...
- Worktree creation mechanics (`git worktree add` at `${canonical.root}/.worktrees/work-N.NN-<kebab>`, branch naming, base-at-canonical-main-HEAD) — worktrees are created by the orchestrator at round start per §11, BEFORE this skill runs; this eval assumes the worktree pre-exists per the fixture setup. Worktree creation is covered by `tests/test-worktree.sh` and `evals/planning-vertical-slice.md`.
