...
1. **Setup** — orchestrator (or a setup subagent it dispatches) prepares the fixture: tmp dual-repo workspace (canonical + AI workspace siblings with a `.workspace/pairing.json` manifest at the parent), a fully-merged slice at `<ai-workspace>/docs/specs/sprint-3/VS-3.2-<kebab>/` with all 4-5 work-item subdirs containing `spec.md` + `handoff.md` + `report.md`, a VS README with `auto:`/`user:` demo criteria per §14.1 grammar, canonical worktrees at `${canonical.root}/.worktrees/work-3.2.NN-<kebab>` still present (per §11 M2 — worktrees survive round close, removed only at slice close), all work-item branches merged into canonical main, and any scenario-specific handoffs in `<ai-workspace>/.workspace/handoffs/`.
...
- Slice VS-3.2 fully implemented: 4 work-item subdirs under `<ai-workspace>/docs/specs/sprint-3/VS-3.2-<kebab>/`, each with `spec.md` + `handoff.md` + a complete `report.md` (status `complete`, AC outcomes all pass, "Suggestions for memory bank" section populated on 2 of the 4 reports).
- VS README at `<ai-workspace>/docs/specs/sprint-3/VS-3.2-<kebab>/README.md` contains 3 demo criteria per §14.1: two `auto:` lines (e.g., `- [ ] auto: \`pytest tests/integration/test_slice.py\` → expected: exit 0` and `- [ ] auto: \`curl -s localhost:8000/api/health\` → expected: output contains "ok"`) and one `user:` line (e.g., `- [ ] user: Navigate to localhost:3000/dashboard → expected: new card visible`).
- All work-item branches merged into canonical main; canonical worktrees at `${canonical.root}/.worktrees/work-3.2.01-<kebab>` through `work-3.2.04-<kebab>` still present per §11 M2.
- `<ai-workspace>/.workspace/handoffs/` contains 2 handoff files matching the slice: `vs-3.2-bugfix-auth-a1b2.md` (mid-slice bug-fix detour with a populated section 4 "What's NOT in memory bank yet") and `vs-3.2-techdebt-logging-e5f6.md` (likewise with a section-4 promote-candidate). Also present: one unrelated `sprint-3-context-bloat-c3d4.md` that MUST NOT be swept (different scope; not slice-tagged).
...
**Trigger:** target subagent user message: `close VS-3.2`

**Expected behavior:**
- Skill triggers via description-match on the "close VS-3.2" trigger phrase (per SPEC §7.1 + §14 triggers list).
...
- After critic returns control, skill authors `retrospective.md` at `<ai-workspace>/docs/specs/sprint-3/VS-3.2-<kebab>/retrospective.md` per §16b's 7-section format.
- Skill runs the §15.2 8-step harvest: (1) reads all 4 work-item `report.md` files; (2) reads all `vs-3.2-*.md` handoffs at `<ai-workspace>/.workspace/handoffs/` (sweeps the 2 slice-scoped handoffs, skips the unrelated `sprint-3-context-bloat-c3d4.md`); (3) extracts report "Suggestions for memory bank" sections + handoff section 4 promote-candidates; (4) categorizes by target memory-bank file; (5) surfaces each candidate to the user with source-tag prefix `[report]` or `[handoff]`; (6) consumes user accept/edit/reject decisions; (7) applies approved items to the relevant memory-bank files with the provenance trailer `<!-- Added from VS-3.2 retrospective, YYYY-MM-DD; source: report | handoff -->`; (8) records harvest outcomes in `retrospective.md` (including which handoff items were promoted vs left in handoff).
...
- Skill emits final "VS-3.2 closed" handoff message.

**Assertion (judge subagent verifies):**
- Target subagent's tool-call log contains exactly 2 Bash invocations running the 2 `auto:` demo commands. Both invocations execute in canonical (NOT in any `.worktrees/work-3.2.NN-<kebab>` path) — the judge confirms either the absence of a `cd <worktree>` prefix or the explicit presence of a canonical-root cwd marker.
...
- Target subagent's tool-call log contains a `Write` (or equivalent) of `<ai-workspace>/docs/specs/sprint-3/VS-3.2-<kebab>/retrospective.md`, and that Write appears AFTER the architect-critic invocation in tool-call order.
- Harvest reads: target subagent's tool-call log contains Reads of all 4 work-item `report.md` files AND Reads of both `vs-3.2-bugfix-auth-a1b2.md` and `vs-3.2-techdebt-logging-e5f6.md`. The unrelated `sprint-3-context-bloat-c3d4.md` MUST NOT appear in any Read (handoff sweep is slice-scoped).
...
- Approved harvest items applied to memory-bank files include the literal provenance trailer string matching `<!-- Added from VS-3.2 retrospective, <date>; source: report -->` or `<!-- Added from VS-3.2 retrospective, <date>; source: handoff -->` (judge accepts minor date-format variation; rejects: missing trailer, missing `source:` field, missing `VS-3.2` reference).
- **Worktree removal ordering (M2 enforcement):** any `git worktree remove` Bash invocation MUST appear AFTER the `retrospective.md` Write AND after the harvest-step Reads of report/handoff files. Judge scans the tool-call log: if any `git worktree remove` or `git branch -D work-3.2.*` invocation precedes the retrospective Write or precedes any harvest Read, the scenario FAILS.
- All 4 work-item worktrees ARE removed by ceremony end (final filesystem state shows `${canonical.root}/.worktrees/work-3.2.01-<kebab>` through `work-3.2.04-<kebab>` absent), and all 4 work-item branches ARE deleted (final `git branch` listing in canonical does not contain any `work-3.2.NN-*` branch).
- Target subagent's final assistant message indicates the slice is closed (e.g., "VS-3.2 closed" or equivalent).
...
**Setup:**
- Dual-repo fixture identical to S1: manifest present, slice VS-3.2 implemented, all branches merged, worktrees present at `${canonical.root}/.worktrees/work-3.2.NN-<kebab>`, architect-critic cache stub present, handoffs identical to S1.
...
- No Reads of `<ai-workspace>/.workspace/handoffs/vs-3.2-*.md` appear in the transcript (harvest is downstream of critic; halt was upstream).
- **Worktrees preserved (M2 enforcement):** no `git worktree remove` Bash invocation appears in the tool-call log. Final filesystem state confirms `${canonical.root}/.worktrees/work-3.2.01-<kebab>` through `work-3.2.04-<kebab>` all still present. No `git branch -D work-3.2.*` invocation appears either.
...
**Setup:**
- Dual-repo fixture identical to S1: manifest present, slice VS-3.2 implemented, all branches merged, worktrees present, handoffs at S1's positions, VS README with 2 `auto:` (both pass) + 1 `user:` line.
...
- Target subagent's tool-call log contains a `Write` of `<ai-workspace>/docs/specs/sprint-3/VS-3.2-<kebab>/retrospective.md` (ceremony proceeded; absence did not block).
- Target subagent's tool-call log contains Reads of all 4 work-item `report.md` files and both `vs-3.2-*.md` handoffs (harvest still runs).
...
- Dual-repo fixture identical to S1 happy path (architect-critic cache present, both `auto:` pass, `user:` will pass), but the report/handoff content is curated so harvest produces a mix of both source-tags:
  ...
  - `<ai-workspace>/.workspace/handoffs/vs-3.2-bugfix-auth-a1b2.md` section 4 ("What's NOT in memory bank yet") contains 1 promote-worthy item (e.g., "auth retry pattern not yet codified").
  - `<ai-workspace>/.workspace/handoffs/vs-3.2-techdebt-logging-e5f6.md` section 4 contains 1 promote-worthy item (e.g., "log-rotation cron caveat not yet codified").
  - A 3rd handoff `sprint-3-context-bloat-c3d4.md` exists but is sprint-scoped (not slice-scoped); MUST NOT be swept.
...
**Assertion (judge subagent verifies):**
- Target subagent's tool-call log shows Reads of all 4 work-item `report.md` files (harvest step 1) AND Reads of exactly `vs-3.2-bugfix-auth-a1b2.md` + `vs-3.2-techdebt-logging-e5f6.md` (harvest step 2 — slice-scoped sweep). The unrelated `sprint-3-context-bloat-c3d4.md` MUST NOT appear in any Read.
