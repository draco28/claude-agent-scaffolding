...
### Added
- **#48 Part F — `sd_harvest_lint_length` lean-index length leg + harvest §9.4 restate-prevention check.** The linter now flags memory-bank entries that exceed the length ceiling, complementing the existing duplication/vagueness legs. The §9.4 check prevents re-stating content already present in the target file (anti-bloat gate).
...
- **#5 — `executing-work-item` §3.6 pre-flight RED-tests gate ("not-already-GREEN" semantics) + `sd_redgate_assert_red` helper + thrust-0 `--allow-skip-thrust-zero` skip-escape.** Tests that are meant to turn green during implementation must be confirmed RED (or missing) before work begins; the gate hard-fails if a target test is already green, preventing phantom "passes". The `--allow-skip-thrust-zero` flag allows the gate to be skipped for thrust-0 scaffolding tasks where no RED test yet exists.
