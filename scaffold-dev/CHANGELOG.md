...
### Added
- **#82 — finding-disposition loop + severity bar (Gap 1).** The agent-driven pre-merge gate in `planning-vertical-slice/references/git-workflow.md` (the single source the closing / sprint-retrospective skills cross-reference) now codifies the per-PR resolution loop: every reviewer finding gets a **recorded disposition** before merge — a P1/blocking finding **must be fixed before merge** (not eligible for merge-time deferral), while only non-blocking P2/nit findings are eligible for **fix-or-defer** (defer via `deferring-work-item`, which writes the `[TD] … → #N` index line). A fix pushes a new commit and **confirms the re-review on the new head sha**; per-finding dispositions (`fixed in <sha>` / `deferred → #N`) carry into the surface so the merge decision is auditable.
- **#82 — reviewer-completeness + head-sha staleness detection (Gap 2).** The gate now treats a configured/expected reviewer whose only signal on the head commit is *skipped / disabled / queued / pending / no-verdict* as **absent (not green)**, never as approval — reading the check `description` / review body, not just its `conclusion`. Canonical case documented: **CodeRabbit disables auto-review on any base branch other than the repo default**, so a `slice/* → sprint-N` PR (non-default base) is never auto-reviewed — it posts a `SUCCESS` status with a *"Review skipped"* body without ever reviewing (the `sprint-N → main` PR has the default base and IS auto-reviewed). Remediation: surface for ack and/or trigger the reviewer (`@coderabbitai review` / `@codex review`) and confirm its terminal verdict on the head commit. A review counts only on the current head sha (a verdict on a prior sha is stale).
- **#82 — anti-rot seam-lint.** `tests/test-review-gate.sh` gains `test_seam_premerge_gate_contract`, pinning the new gate clauses against `git-workflow.md` so the (unenforced-by-design) reasoning contract can't silently rot. 11 new assertions.

### Notes
- **Prose-only enhancement — no new `sd` helper.** The agent already receives the needed data in `sd pr_state` (`statusCheckRollup`, review bodies, `commits`) + `sd pr_review_comments`; baking semantic "skip" detection into bash was rejected per the agent-review-over-deterministic-gates principle (`git-workflow.md` keeps deterministic checks only for mechanical git/`gh` facts). The `closing-vertical-slice` and `writing-sprint-retrospective` call-site summaries were tightened to stay accurate.
