...
- **#5 — `executing-work-item` §3.6 pre-flight RED-tests gate ("not-already-GREEN" semantics) + `sd_redgate_assert_red` helper + `--allow-skip-thrust-zero` skip-escape.** Tests that are meant to turn green during implementation must be confirmed RED (or missing) before work begins; the gate hard-fails if a target test is already green, preventing phantom "passes". The `--allow-skip-thrust-zero` flag overrides a legitimate already-GREEN AC (e.g. a pure code-deletion AC).
