...
- **#82 — reviewer-completeness + head-sha staleness detection (Gap 2).** The gate now treats a configured/expected reviewer whose only signal on the head commit is *skipped / disabled / queued / pending / no-verdict* as **absent (not green)**, never as approval — reading the check `description` / review body, not just its `conclusion`. Canonical case documented: **CodeRabbit disables auto-review on any base branch other than the repo default**, so on `pr_hierarchical` PRs (every PR has a non-default base) it posts a `SUCCESS` status with a *"Review skipped"* body without ever reviewing. Remediation: surface for ack and/or trigger the reviewer (`@coderabbitai review` / `@codex review`) and confirm its terminal verdict on the head commit. A review counts only on the current head sha (a verdict on a prior sha is stale).
