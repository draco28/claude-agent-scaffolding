...
### Added
- **`lib/codex.sh` — mechanical adapter for the Codex backend.** `sd_codex_resolve_companion` (locate the installed `codex-companion.mjs`, newest version, with `SCAFFOLD_CODEX_COMPANION` override; fail-loud + remediation), `sd_codex_preflight` (hard gate: `setup --json` availability/auth + worktree-trust path-prefix check; no silent fallback), `sd_codex_dispatch` (`task --background --write --prompt-file`), `sd_codex_wait` (background poll + stall heuristic + wall-cap → `completed|failed|cancelled|stalled|capped|error`; cancels a stalled/capped job), `sd_codex_result` (extract the fenced `{mode,…}` JSON Codex emits), `sd_codex_verify_nocommit` (assert Codex did not commit). All helpers are `set -e`-safe and tested through `bin/sd`.
- **`lib/backend.sh` — `sd_backend_resolve`.** Resolves the backend with precedence per-invocation override > manifest `.implementer_backend` > `claude_subagent`. Read-with-default (no workspace-init schema change); invalid values fail loud.
...
### Changed
- **`planning-vertical-slice` §8.3 — backend-selector dispatch.** Resolves the backend first; `claude_subagent` (§8.3a) keeps the existing `Task(subagent_type="scaffold-dev:implementer-agent")` path; `codex` (§8.3b) runs preflight → prompt-file assembly (contract prompt-carried) → dispatch → background wait → result → no-commit verify, then joins the existing downstream unchanged. Replaces the prior proto-typed "Codex worker subagent" prose. Codex work items dispatch sequentially within a round.
