#!/usr/bin/env node
// Fake codex-companion for scaffold-dev tests (SS-5). Emits env-driven canned
// JSON matching the real codex-plugin-cc `setup|task|status|result|cancel --json`
// output shapes. NO real Codex, NO network, NO node-codex. Argv recorded to
// $CODEX_SHIM_LOG (one space-joined line per call) so tests can assert which
// flags the helpers passed (--write / --background / --prompt-file / cancel …).
//
...
switch (sub) {
  ...
  case "result": {
    const raw =
      ...
      'Work complete.\n\n```json\n{"mode":"complete","report_path":"/tmp/report.md","summary":"did it","stage_status":"all_staged","gaps":[]}\n```\n';
