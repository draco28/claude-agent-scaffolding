...
test_read_cursor() {
  ...
  setup_tmp_workspace
  cd "$TMP_AI_WORKSPACE"
  _mk_active_context "$TMP_AI_WORKSPACE/.claude/memory-bank" 3 "VS-3.2" "2.04"
  local out
  out="$(sd_state_read_cursor)"
  local sprint
  sprint="$(echo "$out" | jq -r .sprint)"
  assert_eq "read_cursor.sprint" "3" "$sprint"
}
...
test_read_cursor_slice() {
  ...
  assert_eq "active_slice" "VS-3.2" "$out"
}
...
test_active_sprint() {
  ...
  setup_tmp_workspace
  cd "$TMP_AI_WORKSPACE"
  _mk_active_context "$TMP_AI_WORKSPACE/.claude/memory-bank" 5 "VS-5.1" "1.01"
  local out
  out="$(sd_state_active_sprint)"
  assert_eq "active_sprint" "5" "$out"
}
...
test_write_cursor_create() {
  ...
  sd_state_write_cursor 4 "VS-4.3" "3.02"
  assert_file_exists "$TMP_AI_WORKSPACE/.claude/memory-bank/05-active-context.md"
  assert_file_contains "$TMP_AI_WORKSPACE/.claude/memory-bank/05-active-context.md" '"sprint":[[:space:]]*"4"'
}
...
test_write_cursor_update() {
  ...
  setup_tmp_workspace
  cd "$TMP_AI_WORKSPACE"
  _mk_active_context "$TMP_AI_WORKSPACE/.claude/memory-bank" 1 "VS-1.1" "1.01"
  sd_state_write_cursor 2 "VS-2.2" "4.05"
  local out
  out="$(sd_state_active_sprint)"
  assert_eq "updated sprint" "2" "$out"
  local slice
  slice="$(sd_state_active_slice)"
  assert_eq "updated slice" "VS-2.2" "$slice"
}
...
test_write_cursor_atomic() {
  ...
  cd "$TMP_AI_WORKSPACE"
  mkdir -p "$TMP_AI_WORKSPACE/.claude/memory-bank"
  sd_state_write_cursor 1 "VS-1.1" "1.01"
  local tmp_count
...
test_concurrent_reads() {
  ...
  setup_tmp_workspace
  cd "$TMP_AI_WORKSPACE"
  _mk_active_context "$TMP_AI_WORKSPACE/.claude/memory-bank" 7 "VS-7.1" "8.01"
  local r1 r2
