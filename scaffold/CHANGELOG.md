...
### Added — Phase I: end-to-end integration suite + bug fixes (2026-04-27)
- `tests/test-e2e.sh`: 41-test integration walk through the complete workflow on two scenarios — a fresh bootstrap repo (init → slice cycle through all 5 phases → governance commands → audit → SessionStart hook → worktree fork) and an existing repo onboarding path (init respects existing files, surfaces existing ADR, audits cleanly). Plus memory-bank smoke (Python-seeded sqlite + hook surfacing).
- **Bug fixes caught by the integration suite (3 real, 1 test-only):**
  - `lib/repo.sh::sf_repo_hash` now uses `git rev-parse --git-common-dir` for the path fallback. Previously: worktrees of a remote-less repo got different hashes (different `--show-toplevel`), so worktree fork's state-inheritance fix didn't reach `sf_state_get` from inside the new worktree. Now: all worktrees of one local clone share the common-dir, so they share state correctly even without a remote URL.
  - `lib/state.sh::sf_init_state` now seeds `adr_counter` from existing `docs/adr/NNNN-*.md` files via new `sf_scan_existing_adrs` helper. Previously: onboarding an existing repo with `0001-*.md` already present, then running `/adr-new` would create another `0001-*.md` collision. Now: counter starts at the highest existing N, so the first new ADR is `NNNN+1`.
  - Test bug E1.7: `jq` path with shell-escaped quotes (`'.stack | join(\",\")')` produced backslash-quote chars that survived bash variable expansion and crashed jq with "INVALID_CHARACTER". Fix: switched test to a non-quoting path (`.stack[0]`). Documents a real footgun for callers of `sf_state_get_path`: prefer paths that don't need string literals; if you must, build the jq program in the calling code rather than passing it through `${path}` interpolation.
- Total scaffold test count: **236 tests** across 9 suites (30 state + 16 claude-md + 21 audit + 36 slice-gates + 23 governance + 28 mcp + 21 worktree + 20 session-start + 41 e2e), all passing.
